...
class ATLASCore:
    ...
    def __init__(self, config: Dict[str, Any], logger: logging.Logger, data_dir: Path):
        ...
        Args:
            ...
            data_dir: Path to the directory where ATLAS should store its data (maps, visualizations).
        """
            ...
            if "nodes" in system_data:
                for node_id, node_data in system_data.get("nodes", {}).items():
                    ...
                    if isinstance(node_data, dict):
                        ...
                        self.logger.warning(f"Skipping node '{node_id}' due to non-dict data: {node_data}")
                        self.graph.add_node(node_id) # Add node without attributes if data is invalid
            else:
            ...
            if "edges" in system_data:
                for edge in system_data.get("edges", []):
                    ...
                    if source not in self.graph:
                         self.logger.warning(f"Source node '{source}' not found in graph for edge {edge}. Adding node.")
                         self.graph.add_node(source)
                    if target not in self.graph:
                         self.logger.warning(f"Target node '{target}' not found in graph for edge {edge}. Adding node.")
    ...
    def visualize(self, output_filename: Optional[str] = None, 
                 title: Optional[str] = None,
                 layout: Optional[str] = None) -> Optional[Path]:
        """
        ...
        try:
            ...
            try:
                if layout_algo == "spring":
                    ...
                    self.logger.warning(f"Unknown layout '{layout_algo}'. Defaulting to spring layout.")
                    pos = nx.spring_layout(self.graph)
            except Exception as layout_e:
                 self.logger.error(f"Error calculating layout '{layout_algo}': {layout_e}. Defaulting to spring layout.")
            ...
            self._log_operation(operation, "Completed", 
    ...
    def export_to_obsidian(self) -> Optional[Tuple[str, Path]]:
        ...
        self._log_operation(operation, "Started", 
                           f"Generating content for Obsidian export")
        ...
        try:
            ...
            if not generated_image_path or not generated_image_path.exists():
                 self._log_operation(operation, "Failed", "Failed to generate visualization image for Obsidian content.")
                 return None
            ...
            self._log_operation(operation, "Failed", 
                               f"Error generating Obsidian content: {str(e)}")
    ...
    def _generate_markdown(self, image_filename: str) -> str:
        ...
        if num_nodes > 0:
            try:
                ...
                central_nodes_str = "\n".join([
                    f"- **{node}**: {score:.2%}" 
                    for node, score in central_nodes[:min(len(central_nodes), 5)] # Show top 5 or fewer
                ]) if central_nodes else "No central nodes found."
...
"""
    ...
    def load_mapping(self, filepath: Path) -> bool:
         ...
         try:
              ...
              self._log_operation(operation, "Failed", f"Invalid JSON in mapping file {filepath}: {json_e}")
    ...
    def analyze_system(self) -> Dict[str, Any]:
        ...
        try:
            self.logger.info(f"Analyzing system with {self.graph.number_of_nodes()} nodes and {self.graph.number_of_edges()} edges")
            ...
            if num_nodes > 1:
                ...
                except Exception as e: self.logger.error(f"Error calculating degree centrality: {e}")
                try:
                     analysis["centrality"]["betweenness"] = nx.betweenness_centrality(self.graph)
                except Exception as e: self.logger.error(f"Error calculating betweenness centrality: {e}")
            ...
            detect_communities_enabled = self.config.get("analysis", {}).get("detect_communities", False)
            if detect_communities_enabled and num_nodes > 2:
                try:
                    ...
                    self.logger.info(f"Detected {num_communities} communities using Louvain method.")
                except ImportError:
                    self.logger.warning("Community detection skipped: 'python-louvain' library not installed. Run 'pip install python-louvain'.")
            ...
            self._log_operation(operation, "Failed", 
                               f"Error during system analysis: {str(e)}")
