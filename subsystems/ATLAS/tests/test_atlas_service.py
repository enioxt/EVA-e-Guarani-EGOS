...
def test_config(tmp_path: Path) -> Dict:
    ...
    return {
        ...
        "log_level": "DEBUG",
        "core_config": { # Config for ATLASCore
            "visualization": {"default_format": "svg"}
        }
    }
...
def test_service_initialization(mock_atlas_core_cls, test_config, mock_mycelium, project_root, mock_atlas_core):
...
@pytest.mark.asyncio
@patch("subsystems.ATLAS.service.ATLASCore") 
async def test_service_start_and_subscribe(mock_atlas_core_cls, test_config, mock_mycelium, project_root):
    ...
    assert mock_mycelium.subscribed_topics[expected_topics[0]][0] == service.handle_map_system_request
    assert mock_mycelium.subscribed_topics[expected_topics[1]][0] == service.handle_generate_obsidian_request
    assert mock_mycelium.subscribed_topics[expected_topics[2]][0] == service.handle_analyze_system_request
...
@pytest.mark.asyncio
@patch("subsystems.ATLAS.service.ATLASCore") 
async def test_handle_map_system_request(mock_atlas_core_cls, test_config, mock_mycelium, project_root, mock_atlas_core):
    ...
    request_message = {
        "id": "map-req-1",
        "payload": {
            "system_data": {"nodes": {"X": {}}, "edges": []}, 
            "map_name": "MySystemMap"
        }
    }
    ...
    mock_atlas_core.map_system.assert_called_once_with(
        request_message["payload"]["system_data"],
        request_message["payload"]["map_name"]
    )
...
@pytest.mark.asyncio
@patch("subsystems.ATLAS.service.ATLASCore") 
async def test_handle_map_system_request_error(mock_atlas_core_cls, test_config, mock_mycelium, project_root, mock_atlas_core):
...
@pytest.mark.asyncio
@patch("subsystems.ATLAS.service.ATLASCore") 
async def test_handle_generate_obsidian_request(mock_atlas_core_cls, test_config, mock_mycelium, project_root, mock_atlas_core):
...
@pytest.mark.asyncio
@patch("subsystems.ATLAS.service.ATLASCore") 
async def test_handle_analyze_system_request(mock_atlas_core_cls, test_config, mock_mycelium, project_root, mock_atlas_core):
