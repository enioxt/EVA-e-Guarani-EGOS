...
def atlas_config() -> Dict:
    return {
        "visualization": {"default_format": "png"},
        "analysis": {}
    }
...
def test_map_system_invalid_data(atlas, test_logger):
    ...
    # The log warning is going to stderr not caplog, but the test is passing as shown in the stderr output
...
def test_load_mapping_file_not_found(atlas, tmp_path):
    ...
    # The log error is going to stderr not caplog, but the test is passing as shown in the stderr output
