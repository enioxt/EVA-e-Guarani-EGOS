...
class MyceliumClient:
    ...
    async def trigger_message(self, topic: str, message_data: dict):
        ...
        if topic in self.subscriptions:
            message = Message(
                id="test_msg_" + datetime.now().isoformat(),
                data=message_data
            )
...
async def test_relationship_update_error_handling(cartographer, mock_mycelium):
    ...
    cartographer.update_relationship = AsyncMock(side_effect=ValueError("Invalid relationship data"))
...
async def test_generate_map_complex(cartographer):
    ...
    cartographer.relationships = {
        ...
        "svc_b": [
            {"source": "svc_b", "target": "db_a", "type": "uses"}
        ]
    }
    cartographer.metadata = {
        "root": {"version": "1.0"},
...
async def test_update_metadata_existing(cartographer):
    ...
    # Perform update
    await cartographer.update_metadata(component, {"version": "1.1", "status": "stable", "owner": "team_a"})
...
async def test_update_relationship_existing(cartographer):
    ...
    expected_relationship = {"source": source, "target": target, "type": rel_type, "metadata": update_metadata}
...
async def test_update_relationship_new(cartographer):
    ...
    rel_type = "calls"
    metadata = {"async": True}
    expected_relationship = {"source": source, "target": target, "type": rel_type, "metadata": metadata}
...
async def test_cache_miss_expired(mock_dt, cartographer):
    ...
    fresh_map_result = {"nodes": {"expired_target": {}, "new_node": {}}, "relationships": [], "metadata": {}}
    
    # Mock time
    mock_dt.now.return_value = initial_time
    
    # Prime the cache
    cartographer.analysis_cache[cache_key] = {"result": cached_map_result, "timestamp": initial_time}
