...
import json # For potential config loading/saving
...
class AtlasService:
    ...
    def __init__(self, config: Dict[str, Any], mycelium_interface: MyceliumInterface, project_root: Path):
        ...
        # self.atlas_core_logger = logging.getLogger("EGOS.ATLAS") # Match name used in core - Now using Koios
        ...
        #      self.atlas_core_logger.propagate = False # Avoid double logging if root logger has handlers
        # ---------------------------------
    ...
    async def start(self):
        ...
        try:
            ...
            await self.interface.subscribe(f"request.{self.node_id}.map_system", self.handle_map_system_request)
            # Example: Listen for requests to generate Obsidian content
            await self.interface.subscribe(f"request.{self.node_id}.generate_obsidian", self.handle_generate_obsidian_request)
             # Example: Listen for requests to analyze the current map
            await self.interface.subscribe(f"request.{self.node_id}.analyze_system", self.handle_analyze_system_request)
    ...
    async def handle_map_system_request(self, message: Dict[str, Any]):
        ...
        try:
            ...
            await self.interface.publish(response_topic, {"type": "map_system_response", "payload": response_payload})
            self.logger.info(f"Processed map_system request {request_id}. Success: {success}") # Use self.logger

        except Exception as e:
            self.logger.error(f"Error handling map_system request {request_id}: {e}", exc_info=True) # Use self.logger
    ...
    async def handle_generate_obsidian_request(self, message: Dict[str, Any]):
        ...
        try:
            ...
            if result:
                ...
                self.logger.info(f"Generated Obsidian content successfully for {request_id}. Image: {image_path}") # Use self.logger
            else:
                response_payload = {"success": False, "message": "Failed to generate Obsidian content."}
                self.logger.warning(f"Failed to generate Obsidian content for {request_id}.") # Use self.logger

            await self.interface.publish(response_topic, {"type": "generate_obsidian_response", "payload": response_payload})

        except Exception as e:
            self.logger.error(f"Error handling generate_obsidian request {request_id}: {e}", exc_info=True) # Use self.logger
            await self.interface.publish(response_topic, {"type": "error", "payload": {"message": str(e)}})
    ...
    async def handle_analyze_system_request(self, message: Dict[str, Any]):
        ...
        try:
             ...
             await self.interface.publish(response_topic, {"type": "analyze_system_response", "payload": response_payload})
             self.logger.info(f"Processed analyze_system request {request_id}. Success: {response_payload['success']}") # Use self.logger
             
        except Exception as e:
            self.logger.error(f"Error handling analyze_system request {request_id}: {e}", exc_info=True) # Use self.logger
            await self.interface.publish(response_topic, {"type": "error", "payload": {"message": str(e)}})
...
async def main():
    ...
    config = {
         ...
         "core_config": { # Config specific to ATLASCore
              ...
              "analysis": {
                   "detect_communities": False # Example: disable community detection
              }
         }
    }
    # ---------------------------
