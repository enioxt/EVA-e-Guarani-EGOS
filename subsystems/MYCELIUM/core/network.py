...
class MyceliumNetwork:
    ...
    def __init__(self):
        ...
        self.connections: Dict[str, Set[str]] = defaultdict(set) # node_id -> set of connected node_ids
        self.subscriptions: Dict[str, List[tuple[str, Callable[[Dict[str, Any]], Coroutine]]]] = defaultdict(list) # topic -> list of (node_id, async_callback)
        ...
        self._response_handlers: Dict[str, Callable] = {} # node_id -> _handle_response method from interface
    ...
    async def register_node(self, node_id: str, node_type: str, version: str, capabilities: List[str]) -> bool:
    ...
    async def remove_node(self, node_id: str) -> bool:
        ...
        for topic in list(self.subscriptions.keys()):
            self.subscriptions[topic] = [(sub_id, cb) for sub_id, cb in self.subscriptions[topic] if sub_id != node_id]
    ...
    def add_connection(self, node1_id: str, node2_id: str):
        ...
        if node1_id in self.nodes and node2_id in self.nodes:
            ...
            logger.error(f"Cannot add connection: one or both nodes not found ({node1_id}, {node2_id})")
    ...
    async def add_subscription(self, topic: str, node_id: str, callback: Callable[[Dict[str, Any]], Coroutine]):
        ...
        if not any(sub_id == node_id and cb == callback for sub_id, cb in self.subscriptions[topic]):
            self.subscriptions[topic].append((node_id, callback))
            logger.info(f"Node {node_id} subscribed to topic: {topic}")
        else:
            logger.warning(f"Node {node_id} already subscribed to topic {topic} with this callback.")
    ...
    async def _handle_single_message(self, message: Dict[str, Any]):
        ...
        try:
            ...
            if not all([msg_type, target, topic, sender]):
                logger.error(f"Message header missing required fields (type, target, topic, sender): {header}")
                return

            logger.debug(f"Processing message {msg_id} from {sender} to {target} ({topic}) [{msg_type}] ")
            ...
            if msg_type == "RESPONSE":
                ...
                if response_target_node in self._response_handlers:
                     try:
                         ...
                         logger.error(f"Error invoking response handler for node {response_target_node}, corr_id {correlation_id}: {e}", exc_info=True)
                else:
                    logger.warning(f"No response handler registered for node {response_target_node} to handle corr_id {correlation_id}")

            # --- REQUEST Handling --- #
            elif msg_type == "REQUEST":
                if target not in self.nodes:
                    logger.error(f"Cannot route REQUEST: Target node {target} not found.")
                    error_payload = {"status": "ERROR", "error_message": f"Target node '{target}' not found"}
                    response_msg = self._create_response_message(message, error_payload)
                    if response_msg: await self.route_message(response_msg)
                    return
                ...
                try:
                    ...
                    logger.error(f"Error processing REQUEST in node {target} for topic {topic}: {e}", exc_info=True)
                    error_payload = {"status": "ERROR", "error_message": f"Error processing request in {target}: {str(e)}"}
                    response_msg = self._create_response_message(message, error_payload)
                    if response_msg: await self.route_message(response_msg)
            ...
            # --- EVENT Handling --- #
                ...
                if target == "BROADCAST":
                    subscribers_to_notify = [(nid, node) for nid, node in self.nodes.items() if nid != sender]
                elif target == "TOPIC_TARGET":
                    subscribers_to_notify = [(sub_id, self.nodes[sub_id]) for sub_id, cb in self.subscriptions.get(topic, []) if sub_id in self.nodes]
                ...
                for node_id, node_instance in subscribers_to_notify:
                    if node_id != sender:
                        try:
                            ...
                            if asyncio.iscoroutinefunction(handler_coro) or isinstance(handler_coro, Coroutine):
                                asyncio.create_task(handler_coro(message), name=f"event_callback_{node_id}_{topic}")
                            else:
                                 logger.error(f"Invalid handler for event callback node {node_id} on topic {topic}: {type(handler_coro)}")
                        except Exception as e:
                            logger.error(f"Error scheduling/executing EVENT callback for node {node_id} on topic {topic}: {e}", exc_info=True)
            else:
    ...
    def _create_response_message(self, request_message: Dict[str, Any], response_payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if not isinstance(header, dict) or not all(k in header for k in ["correlation_id", "target_node", "sender_node", "topic"]):
        ...
        return {
            "header": {
                ...
                "sender_node": request_message["header"]["target_node"], # Response comes from original target
                "target_node": request_message["header"]["sender_node"], # Send back to original sender
    ...
    async def start(self):
        ...
        if self._message_processor_task is None or self._message_processor_task.done():
            self._message_processor_task = asyncio.create_task(self._process_messages(), name="mycelium_message_processor")
    ...
    def get_network_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "subscriptions": {topic: [sub[0] for sub in subs] for topic, subs in self.subscriptions.items()},
            "queue_size": self.message_queue.qsize(),
            "processor_running": self._message_processor_task is not None and not self._message_processor_task.done()
        }
