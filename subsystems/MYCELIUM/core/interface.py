...
class MyceliumInterface:
    ...
    async def send_request(
        ...
        logger.debug(
            f"[{self._node_id}] Sending request {correlation_id} to {target_node} on topic {topic}"
        )
        ...
        try:
            ...
            if response_payload.get("status") == "ERROR":
                ...
                raise Exception(
                    f"Error response from {target_node}: {response_payload.get('error_message', 'Unknown error')}"
                )
            ...
            logger.error(
                f"[{self._node_id}] Timeout waiting for response {correlation_id} from {target_node} on topic {topic}"
            )
    ...
    async def _handle_response(self, message: Dict[str, Any]):
        ...
        if correlation_id in self._response_waiters:
            ...
            if not future.done():
                ...
                logger.warning(
                    f"[{self._node_id}] Future for correlation_id {correlation_id} was already done."
                )
        else:
            logger.warning(
                f"[{self._node_id}] Received response for unknown/timed-out/cancelled correlation_id: {correlation_id}"
            )
    ...
    async def register_file(
        ...
        # For now, just log.
        logger.warning(
            f"Placeholder: Node {self._node_id} requested registration for file {file_id}"
        )
