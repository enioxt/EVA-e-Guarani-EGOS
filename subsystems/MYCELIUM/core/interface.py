...
class MyceliumInterface:
    ...
    def __init__(self, network_instance: 'MyceliumNetwork', node_id: str):
        if network_instance is None:
             ...
             raise ValueError("MyceliumNetwork instance is required")
        if not node_id:
            raise ValueError("Node ID is required for Mycelium Interface")
        ...
        logger.debug(f"MyceliumInterface initialized for node: {self._node_id}")

    async def connect(self, node_type: str, version: str, capabilities: List[str]) -> bool:
        """Registers the subsystem node with the network."""
        logger.info(f"Node {self._node_id} connecting to Mycelium Network...")
        # Called by the subsystem during its initialization
        success = await self._network.register_node(self._node_id, node_type, version, capabilities)
        if success:
            # Register handler for incoming responses directed to this node
            await self._network.register_response_handler(self._node_id, self._handle_response)
        return success

    async def disconnect(self) -> bool:
        """Deregisters the subsystem node."""
        logger.info(f"Node {self._node_id} disconnecting from Mycelium Network...")
        ...
        await self._network.remove_response_handler(self._node_id)
        # Remove node from network
        success = await self._network.remove_node(self._node_id)
        self._node_id = None # Clear node id after disconnection
        return success
    ...
    async def send_request(self, target_node: str, topic: str, payload: Dict[str, Any], timeout: int = 10) -> Dict[str, Any]:
        ...
        logger.debug(f"[{self._node_id}] Sending request {correlation_id} to {target_node} on topic {topic}")
        ...
        try:
            ...
            logger.debug(f"[{self._node_id}] Received response for {correlation_id}")
            ...
            if response_payload.get("status") == "ERROR":
                 ...
                 raise Exception(f"Error response from {target_node}: {response_payload.get('error_message', 'Unknown error')}")
            return response_payload
        except asyncio.CancelledError:
            logger.warning(f"[{self._node_id}] Request {correlation_id} cancelled.")
            # Reraise CancelledError if needed, or handle as ConnectionAbortedError
            raise ConnectionAbortedError(f"Request {correlation_id} cancelled due to node disconnection.")
        except asyncio.TimeoutError:
             logger.error(f"[{self._node_id}] Timeout waiting for response {correlation_id} from {target_node} on topic {topic}")
    ...
    async def _handle_response(self, message: Dict[str, Any]):
        ...
        if correlation_id in self._response_waiters:
            future = self._response_waiters.pop(correlation_id) # Remove here to prevent setting twice
            if not future.done():
                logger.debug(f"[{self._node_id}] Handling response for {correlation_id}")
                ...
                future.set_result(payload)
                 ...
                 logger.warning(f"[{self._node_id}] Future for correlation_id {correlation_id} was already done.")
        else:
            logger.warning(f"[{self._node_id}] Received response for unknown/timed-out/cancelled correlation_id: {correlation_id}")
    ...
    async def publish_event(self, topic: str, payload: Dict[str, Any]):
        ...
        logger.debug(f"[{self._node_id}] Publishing event to topic {topic}")
    ...
    async def subscribe(self, topic: str, callback_function: Callable[[Dict[str, Any]], Coroutine]):
        ...
        logger.info(f"[{self._node_id}] Subscribing to topic: {topic}")
    ...
    async def report_health(self, status: str, details: Optional[Dict[str, Any]] = None):
        ...
        logger.debug(f"[{self._node_id}] Reporting health: {status}")
    ...
    async def register_file(self, file_id: str, file_path: str, file_type: str, connections: List[str] = None, version: str = None):
        """(TODO) Registers a file with the Mycelium Network.
        Placeholder - functionality might live elsewhere or interact differently
        with the Python message bus compared to the SLOP API.
        """
        # This needs careful consideration: Does the interface handle this directly,
        # or does it send a message to a specific node (e.g., KOIOS or CRONOS)
        # to handle file registration?
        # For now, just log.
        logger.warning(f"Placeholder: Node {self._node_id} requested registration for file {file_id}")
        # Example message sending to KOIOS (conceptual):
        # await self.send_request(
        #    target_node="KOIOS",
        #    topic="request.koios.register_file",
        #    payload={"file_id": file_id, "file_path": file_path, "file_type": file_type, "connections": connections, "version": version}
        # )
        pass 