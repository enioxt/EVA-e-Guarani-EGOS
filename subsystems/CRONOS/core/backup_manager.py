...
class BackupManager:
    ...
    def __init__(self, project_root: Path, config_path: Optional[Path] = None, mycelium_client: Optional[MyceliumClient] = None):
        ...
        self.logger.info(f"BackupManager initialized with root: {project_root}, backup dir: {self.backup_dir}")
    ...
    async def _publish_alert(self, level: str, message: str, details: Optional[Dict[str, Any]] = None):
    ...
    async def create_backup(self, name: str, backup_type: str = "manual", 
        ...
        if exclude_patterns is None:
            exclude_patterns = [".venv/*", "__pycache__/*", "*.pyc", ".git/*", 
                              "node_modules/*", "backups/*", "logs/*", "data/*"]
        ...
        try:
            with zipfile.ZipFile(backup_path, 'w', 
                               ...
                               compresslevel=self.config["backup"]["compression_level"]) as zipf:
                ...
                for item_path in self.project_root.rglob("*"):
                    try:
                        ...
                        if item_path.is_file() and self._should_include(relative_path, include_patterns):
                        ...
                        await self._publish_alert(
                            "warning",
                            f"Error processing file during backup",
    ...
    def _find_backup_path(self, backup_identifier: str) -> Optional[Path]:
        ...
        if latest_match:
            self.logger.info(f"Found latest backup matching identifier '{backup_identifier}': {latest_match.name}")
            return latest_match
        else:
            self.logger.error(f"No backup found matching identifier: '{backup_identifier}' in {self.backup_dir}")
            return None
    ...
    async def restore_backup(
        ...
        Args:
            backup_identifier (str): The filename or a unique part (like timestamp YYYYMMDD_HHMMSS) 
                                     of the backup zip file.
            restore_target_path (Optional[str]): The directory where the backup should be restored.
                                               Required for 'overwrite' strategy. If None for 'new_location',
                                               a new timestamped directory is created relative to the project root.
            strategy (str): The restore strategy ('new_location', 'overwrite'). Defaults to config or 'new_location'.
        ...
        resolved_strategy = strategy or self.config.get("restore", {}).get("default_strategy", "new_location")
        self.logger.info(f"Initiating restore: ID='{backup_identifier}', Strategy='{resolved_strategy}', Target='{restore_target_path or 'Default New Location'}'")

        # --- 1. Find the Backup File --- 
        backup_path = self._find_backup_path(backup_identifier)
        if not backup_path:
            err_msg = f"Backup '{backup_identifier}' not found."
            await self._publish_alert("error", err_msg, {"backup_id": backup_identifier}) # Publish error
            ...
            if resolved_strategy == "new_location":
                if restore_target_path:
                    ...
                    if target_path.exists() and any(target_path.iterdir()):
                        err_msg = f"Target path '{target_path}' for 'new_location' strategy must not exist or be empty."
                        self.logger.error(err_msg)
                        await self._publish_alert("error", err_msg, {"backup_id": backup_identifier, "target": str(target_path)}) # Publish error
                        return False, err_msg
                else:
                    ...
                    self.logger.info(f"No target path provided for 'new_location', using default: {target_path}")
                ...
                self.logger.warning(f"Restore target (overwrite): {target_path}. Existing files may be overwritten!")
                ...
                if self.config.get("restore", {}).get("create_restore_point", False):
                    ...
                    exclude_patterns_rp = self.config.get("backup", {}).get("exclude_patterns") + [self.backup_dir.name + "/*"]
                ...
                err_msg = f"Unsupported restore strategy: '{resolved_strategy}'. Use 'new_location' or 'overwrite'."
                self.logger.error(err_msg)
                await self._publish_alert("error", err_msg, {"backup_id": backup_identifier}) # Publish error
                return False, err_msg
        ...
        try:
            with zipfile.ZipFile(backup_path, 'r') as zipf:
                ...
                self.logger.info(f"Found {total_files} items in backup (including potential metadata file).")
            ...
            msg = f"Successfully restored backup '{backup_identifier}' to '{target_path}'. Extracted {extracted_count} items."
            self.logger.info(msg)
            await self._publish_alert("info", msg, {"backup_id": backup_identifier, "target": str(target_path)}) # Publish success alert
            return True, msg

        except zipfile.BadZipFile:
            err_msg = f"Error: Backup file '{backup_path.name}' is corrupted or not a valid zip file."
            self.logger.error(err_msg)
            await self._publish_alert("error", err_msg, {"backup_id": backup_identifier}) # Publish error alert
            return False, err_msg
    ...
    async def clean_old_backups(self):
        ...
        self.logger.info(f"Cleaning old backups... Retention Days: {retention_days}, Max Backups: {max_backups}")
        ...
        try:
            ...
            self.logger.info(f"Backup cleanup finished. Deleted: {deleted_count}, Kept: {kept_count}")
    ...
    def list_backups(self) -> List[Dict[str, Any]]:
        ...
        Returns:
            List[Dict[str, Any]]: A list of dictionaries, each representing a backup.
                                   Example: [{'filename': 'backup_..ts.zip', 'size': 1024, 'created_at': 'iso_timestamp'}]
        """
            ...
            for backup_path in backup_files:
                try:
                    ...
                    backups_info.append({
                        ...
                        "created_at": datetime.datetime.fromtimestamp(stat_info.st_mtime).isoformat()
                    })
                except Exception as stat_e:
                    self.logger.warning(f"Could not get info for backup file {backup_path.name}: {stat_e}")
    ...
    async def run_example():
        try:
            ...
            class MockMycelium:
                ...
                def subscribe(self, *args, **kwargs): return lambda func: func
            ...
            if backup_path:
                print(f"✅ Backup concluído com sucesso: {backup_path}")
                ...
                success, msg = await manager.restore_backup(backup_identifier=backup_path.stem) # Use stem as identifier
            ...
            print("✅ Limpeza de backups antigos concluída conforme política de retenção")
        except Exception as e:
            logger.error(f"Example run failed: {e}", exc_info=True)
            print(f"❌ Falha no exemplo: {e}")
