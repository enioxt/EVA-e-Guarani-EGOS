...
def check_dependencies():
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                if "Version:" in result.stdout:
                    logger.warning("⚠️ python-telegram-bot is installed but not importable. Check your Python path.")
                    # Try to install it again to make sure it's in the correct path
                    logger.warning("Attempting to reinstall python-telegram-bot...")
                    subprocess.run(["pip", "install", "--force-reinstall", "python-telegram-bot==13.15"], capture_output=True)
                    # Try importing again
                    try:
                        import telegram
                        logger.info(f"✅ python-telegram-bot found after reinstall (version: {telegram.__version__})")
                    except ImportError:
        ...
        try:
            ...
            logger.warning("⚠️ PaymentGateway module not found. Payment features may not work correctly.")
...
def configure_bot():
    ...
    allowed_users = input(f"Enter allowed user IDs (comma-separated) [current: {config.get('allowed_users')}]: ").strip()
    if allowed_users:
        try:
            config["allowed_users"] = [int(uid.strip()) for uid in allowed_users.split(",") if uid.strip()]
        except ValueError:
            logger.error("Invalid user IDs. Using existing values.")
    
    # Configure admin users
    admin_users = input(f"Enter admin user IDs (comma-separated) [current: {config.get('admin_users')}]: ").strip()
    if admin_users:
        try:
            config["admin_users"] = [int(uid.strip()) for uid in admin_users.split(",") if uid.strip()]
        except ValueError:
...
def main():
    ...
    parser.add_argument("--test-connection", action="store_true", help="Test connection to Telegram")
