                            ...
                            for subitem in item.glob('*'):
                                ...
                                if not target.exists():
                                    try:
                                        ...
                                        logger.error(f"Error merging {subitem} into {target}: {str(e)}")
                        else:
                            # Subdirectory does not exist in the destination, copy directly
                            try:
                                shutil.copytree(item, dest_path / item.name)
                            except Exception as e:
                                logger.error(f"Error copying directory {item} to {dest_path}: {str(e)}")
                    else:
...
def generate_report():
    ...
    try:
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            quarantine_dirs = ["quarentena_20250319", "quarentena_duplicados_20250319", "quarentena_pastas_20250319", "quarantine"]
            for qdir in quarantine_dirs:
                source_path = ROOT_DIR / qdir
                if not source_path.exists():
