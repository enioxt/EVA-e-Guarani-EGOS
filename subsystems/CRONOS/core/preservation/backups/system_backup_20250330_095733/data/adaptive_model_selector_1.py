...
Module responsible for the intelligent selection of models based on the complexity
of the query, usage context, and available resources, aiming to optimize:
- Cost savings
...
class ModelProfile:
    ...
    def __init__(self, name: str, capabilities: Dict[str, float], cost_per_1k_tokens: float, 
                 context_length: int, latency: float, specialties: List[str] = None):
        self.name = name
    ...
    def specialty_score(self, query_topics: List[str]) -> float:
        ...
        matches = sum(1 for topic in query_topics if any(specialty in topic.lower() for specialty in self.specialties))
...
class AdaptiveModelSelector:
    ...
    def __init__(self, config_path: str = "config/model_profiles.json", 
                default_model: str = "gpt-4o", cache_dir: str = "data/model_selection"):
    ...
    def _load_model_profiles(self) -> Dict[str, ModelProfile]:
        ...
        try:
            if os.path.exists(self.config_path):
                ...
                logger.warning(f"Configuration file {self.config_path} not found. Using default profiles.")
    ...
    def analyze_query_complexity(self, query: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        complexity_indicators = {
            "reasoning": sum(query.lower().count(word) for word in 
                          ["why", "because", "how", "explain", "compare", "analyze", "evaluate"]),
            "creativity": sum(query.lower().count(word) for word in 
                           ["create", "invent", "imagine", "develop", "design", "creative"]),
            "knowledge": sum(query.lower().count(word) for word in 
                          ["what is", "who was", "when", "where", "define", "describe"]),
            "coding": sum(query.lower().count(word) for word in 
                       ["code", "function", "program", "bug", "error", "debug", "optimize", "refactor"]),
            "ethics": sum(query.lower().count(word) for word in 
                       ["ethical", "moral", "right", "wrong", "principle", "value", "impact"])
        }
    ...
    def _extract_topics(self, query: str) -> List[str]:
        ...
        # Simplified implementation - in a real system would use more advanced NLP
        ...
        topic_keywords = {
            ...
            "creativity": ["creative", "art", "design", "aesthetics", "innovation", "idea"],
    ...
    def select_model(self, query: str, context: Optional[Dict[str, Any]] = None) -> Tuple[str, Dict[str, Any]]:
        """
        ...
        logger.info(f"Selected model: {model_name} (complexity: {complexity_analysis['complexity_score']:.2f})")
    ...
    def save_stats(self) -> None:
        ...
        stats_file = os.path.join(self.cache_dir, f"selection_stats_{datetime.datetime.now().strftime('%Y%m%d')}.json")
