...
async def metrics_middleware(request: Request, handler: Callable) -> Response:
    ...
    try:
        ...
        # Record metrics
        duration = time.time() - start_time
        metrics_collector.add_metric('http_request_duration_seconds', duration, {
            'method': request.method,
            'path': request.path,
            'status': response.status
        })
        metrics_collector.add_metric('http_requests_total', 1, {
            'method': request.method,
            'path': request.path,
            'status': response.status
        })
        ...
        metrics_collector.add_metric('http_request_failures_total', 1, {
            'method': request.method,
            'path': request.path
        })
        raise
