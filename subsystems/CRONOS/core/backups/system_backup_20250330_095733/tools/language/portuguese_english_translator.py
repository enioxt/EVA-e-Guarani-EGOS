...
def translate_directory(directory: str, extensions: List[str] = None, 
                        rename_files: bool = False, rename_dirs: bool = False, 
                        dry_run: bool = False, max_workers: int = None) -> Dict[str, Any]:
    """
                ...
                if not dry_run:
                    try:
                        ...
                        if os.path.exists(new_dir_path):
                            logger.warning(f"Cannot rename directory {dir_path} to {new_dir_path} - destination exists")
                            stats['errors'].append(f"Rename conflict: {new_dir_path} already exists")
                            continue
                    ...
                    if not dry_run:
                        try:
                            ...
                            if os.path.exists(new_file_path):
                                logger.warning(f"Cannot rename file {file_path} to {new_file_path} - destination exists")
                                stats['errors'].append(f"Rename conflict: {new_file_path} already exists")
                                continue
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="EVA & GUARANI EGOS - Portuguese to English Translator")
    ...
    parser.add_argument("--rename-files", action="store_true", help="Rename files from Portuguese to English")
    parser.add_argument("--rename-dirs", action="store_true", help="Rename directories from Portuguese to English")
    parser.add_argument("--extensions", help="Comma-separated list of file extensions to translate (default: .md,.txt,.py,.js,.json)")
    parser.add_argument("--dry-run", action="store_true", help="Simulate translation without making changes")
