                ...
                try:
                    ...
                    with state_lock:
                        if "cursor" in limits and limits["cursor"].get("measured", False):
                            ...
                            print(f"[MCP] Limite empírico encontrado: {limits['cursor']['limit']} (safe: {limits['cursor']['safe_limit']})")
...
def save_context_limits(platform, limit, measured=True):
    ...
    try:
        ...
        print(f"[MCP] Limite de contexto para {platform} atualizado: {limit} (90% = {int(limit * SAFE_MARGIN)})")
        return True
    except Exception as e:
...
def add_message(content):
    ...
    with state_lock:
        ...
        monitor_state["capacity_used"] = monitor_state["current_size"] / monitor_state["context_limit"]
...
def force_save():
    ...
    try:
        print(f"[MCP Monitor] Executando salvamento forçado às {datetime.datetime.now().strftime('%H:%M:%S')}")
...
def check_context_size():
    ...
    with state_lock:
        ...
        if monitor_state["capacity_used"] >= 0.8 and not monitor_state["threshold_reached"]:
            print(f"[MCP Monitor] Contexto atingiu 80% da capacidade ({monitor_state['capacity_used']*100:.1f}%)")
...
def start_monitoring():
    ...
    if limit_loaded:
        print(f"[MCP Monitor] Usando limite empírico de {monitor_state['context_limit']} caracteres")
    else:
    ...
    print(f"[MCP Monitor] Usando limite de {monitor_state['context_limit']} caracteres (fonte: {monitor_state['context_source']})")
...
if __name__ == "__main__":
    ...
    if load_context_limits():
        print(f"[MCP] Limite carregado do BIOS-Q: {monitor_state['context_limit']} caracteres (empírico)")
    else:
