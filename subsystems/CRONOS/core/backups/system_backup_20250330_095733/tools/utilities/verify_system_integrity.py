...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("system_integrity_check.log"),
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Verify the integrity of the EVA & GUARANI system after reorganization.")
    parser.add_argument(
        "--report", 
        type=str,
...
def check_subcategories(existing_main_categories: List[str]) -> Tuple[bool, Dict[str, List[str]], Dict[str, List[str]]]:
...
def check_moved_folders() -> Tuple[bool, Dict[str, bool]]:
    ...
    for source, destination in MOVED_FOLDERS.items():
        ...
        if destination_exists:
            ...
            logger.error(f"❌ Folder '{destination}' not found. The folder '{source}' may not have been moved correctly.")
            all_moved = False
...
def check_quarantine_folders() -> Dict[str, int]:
    ...
    for folder in quarantine_folders:
        if os.path.isdir(folder):
            ...
            logger.info(f"✅ Quarantine folder '{folder}' exists with {file_count} files and {folder_count} subfolders.")
        else:
            quarantine_info[folder] = {
                "exists": False,
                "file_count": 0,
...
def generate_report(args: argparse.Namespace, results: Dict[str, Any]) -> None:
    ...
    status_ok = results["main_categories_ok"] and results["subcategories_ok"] and results["moved_folders_ok"] and results["critical_files_ok"]
    ...
    with open(args.report, 'w', encoding='utf-8') as f:
        ...
        if status_ok:
            f.write("✅ **SYSTEM INTACT** - All critical components have been verified and are as expected.\n\n")
        else:
            f.write("⚠️ **ATTENTION NEEDED** - Some critical components were not found or have issues.\n\n")
        
        # Summary
        f.write("## Summary\n\n")
        f.write(f"- Main Categories: {'✅ All OK' if results['main_categories_ok'] else '❌ Missing some'}\n")
        f.write(f"- Critical Subcategories: {'✅ All OK' if results['subcategories_ok'] else '❌ Missing some'}\n")
        f.write(f"- Moved Folders: {'✅ All OK' if results['moved_folders_ok'] else '❌ Problems detected'}\n")
        f.write(f"- Critical Files: {'✅ All OK' if results['critical_files_ok'] else '❌ Missing some'}\n\n")
        ...
        for folder, info in results["quarantine_info"].items():
            if info["exists"]:
                f.write(f"- ✅ `{folder}`: {info['file_count']} files, {info['folder_count']} subfolders\n")
            else:
        ...
        if not status_ok:
            ...
            f.write("1. **Update references** in code files to reflect the new directory structure\n")
            f.write("2. **Perform integration tests** to verify that all functionalities remain operational\n")
            f.write("3. **Expand documentation** for components that are not yet fully documented\n")
...
def main() -> None:
    ...
    subcategories_ok, existing_subcategories, missing_subcategories = check_subcategories(existing_categories)
