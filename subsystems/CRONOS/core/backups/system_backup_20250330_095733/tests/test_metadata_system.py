...
class TestMetadataScanner(unittest.TestCase):
    ...
    def test_extract_imports(self):
        ...
        python_imports = self.scanner._extract_imports(
            os.path.join(self.temp_dir, 'test1.py')
        )
        js_imports = self.scanner._extract_imports(
            os.path.join(self.temp_dir, 'test2.js')
        )
        ts_imports = self.scanner._extract_imports(
            os.path.join(self.temp_dir, 'test3.ts')
        )
