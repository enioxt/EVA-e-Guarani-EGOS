...
async def test_list_backups_with_files(backup_manager, temp_backup_dir):
    ...
    create_dummy_backup(temp_backup_dir, "backup_20250401_001", files)
    create_dummy_backup(temp_backup_dir, "backup_20250401_002", files)
    
    backups = await backup_manager.list_backups()
    assert len(backups) == 2
    assert all(b.endswith('.zip') for b in backups)
    assert "backup_20250401_001.zip" in backups
    assert "backup_20250401_002.zip" in backups

@pytest.mark.asyncio
async def test_restore_backup_replace_strategy(backup_manager, temp_backup_dir, temp_project_dir):
    """Test restoring backup with replace strategy"""
...
async def test_restore_backup_merge_strategy(backup_manager, temp_backup_dir, temp_project_dir):
    ...
    files = {
        "new_file1.txt": "backup content 1",
        "new_file2.txt": "backup content 2"
    }
    backup_path = create_dummy_backup(temp_backup_dir, "test_backup", files)
...
async def test_restore_backup_nonexistent_backup(backup_manager, temp_project_dir):
    ...
    with pytest.raises(FileNotFoundError):
        await backup_manager.restore_backup(
            backup_identifier="nonexistent.zip",
            restore_target_path=temp_project_dir,
            strategy="replace"
        )
...
async def test_backup_request_handler(backup_manager_with_mycelium, mycelium_client):
    ...
    status_message = next(
        msg for msg in mycelium_client.published_messages 
        if msg["topic"] == "cronos.backup.status"
    )
...
async def test_restore_request_handler(backup_manager_with_mycelium, mycelium_client):
    ...
    restore_status = next(
        msg for msg in mycelium_client.published_messages 
        if msg["topic"] == "cronos.restore.status"
    )
...
@pytest.fixture
@patch('subsystems.CRONOS.core.backup_manager.KoiosLogger') # Patch KoiosLogger globally for this fixture
def backup_manager(MockKoiosLogger, test_project_root, backup_manager_config, mock_logger):
    ...
    with patch.object(BackupManager, '_load_config', return_value=backup_manager_config):
        manager = BackupManager(project_root=test_project_root, mycelium_client=mock_mycelium_client)
...
@pytest.mark.asyncio
async def test_restore_new_location_specific(backup_manager: BackupManager, test_project_root, tmp_path):
...
@pytest.mark.asyncio
async def test_restore_new_location_target_exists_not_empty(backup_manager: BackupManager, test_project_root, tmp_path):
...
@pytest.mark.asyncio
async def test_restore_overwrite_with_restore_point(backup_manager: BackupManager, test_project_root):
...
async def test_restore_overwrite_no_restore_point(backup_manager: BackupManager, test_project_root):
    ...
    # 3. Restore
    success, msg = await backup_manager.restore_backup(backup_id, str(test_project_root), "overwrite")
    
    # 4. Assertions
    assert success is True
    assert original_file.read_text() == original_content # Overwritten
    # Check that NO restore point was created this time
    assert not any(p.name.startswith("egos_backup_restore_point_") for p in backup_manager.backup_dir.glob("*.zip"))
...
async def test_restore_corrupted_backup(backup_manager: BackupManager, test_project_root):
    ...
    success_no_check, msg_no_check = await backup_manager.restore_backup(backup_identifier=corrupted_zip_path.name)
...
async def test_find_backup_path(backup_manager: BackupManager):
    ...
    assert backup_manager._find_backup_path(timestamp_prefix) == (backup_manager.backup_dir / name2), "Should find latest matching prefix"

    # Test find by unique timestamp
    assert backup_manager._find_backup_path(t3.strftime(ts_format)) == (backup_manager.backup_dir / name3)
...
@pytest.mark.asyncio
async def test_restore_with_integrity_check_disabled(backup_manager: BackupManager, test_project_root):
...
@pytest.mark.asyncio
async def test_restore_with_multiple_matching_backups(backup_manager: BackupManager, test_project_root):
...
async def test_restore_with_special_characters(backup_manager: BackupManager, test_project_root):
    ...
    backup_path = create_dummy_backup(backup_manager.backup_dir, "special_chars_test", special_files)
...
@pytest.mark.asyncio
async def test_restore_with_large_directory_structure(backup_manager: BackupManager, test_project_root):
