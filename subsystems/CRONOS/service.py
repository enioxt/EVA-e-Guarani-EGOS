...
try:
    ...
    from .scripts.verify_cleanup import verify_backup_integrity as verify_integrity_script # Assuming function name
    # Need to refactor verify_cleanup.py into functions first
except ImportError:
    # Fallback if refactoring into this file or structure changes
    logging.warning("Could not import backup/verification scripts directly. Functionality needs integration.")
    BackupManager = None
...
class SystemState:
    ...
    config_hashes: Dict[str, str] = field(default_factory=dict) # e.g., {"config/backup_config.json": "hash..."}
    subsystem_versions: Dict[str, str] = field(default_factory=dict) # e.g., {"CRONOS": "0.1", "KOIOS": "0.2"} (Placeholder for now)
...
class CronosService:
    ...
    def __init__(self, config: Dict[str, Any], mycelium_interface: MyceliumInterface, project_root: Path):
        """Initialize the CRONOS service."""
        self.config = config
        self.interface = mycelium_interface
        self.node_id = "CRONOS"
        self.system_root = project_root
        self.backup_base_path = self.system_root / Path(self.config.get("backup_location", "backups/CRONOS"))
        ...
        logger.info(f"CRONOS Service initialized. Backup location: {self.backup_base_path}")

    async def start(self):
        """Start the CRONOS service and connect to Mycelium."""
        if self._running:
            logger.warning("CRONOS service already running.")
            return

        logger.info("Starting CRONOS service...")
        ...
        try:
             ...
             logger.error(f"Error during initial backup check: {e}", exc_info=True)
        ...
        if not connected:
            logger.error("CRONOS failed to connect to Mycelium Network!")
            return

        # Register message handlers
        # await self.interface.subscribe("request.cronos.create_backup", self.handle_create_backup_request)
        # await self.interface.subscribe("request.cronos.get_status", self.handle_get_status_request)
        ...
        try:
            await self.interface.subscribe(f"request.{self.node_id}.list_backups", self.handle_list_backups_request)
            await self.interface.subscribe(f"request.{self.node_id}.restore_backup", self.handle_restore_backup_request)
            logger.info("Subscribed to backup list/restore request topics.")
        except Exception as e:
             logger.error(f"Failed to subscribe to CRONOS request topics: {e}", exc_info=True)

        self._running = True
        logger.info("CRONOS service started successfully.")
        await self.interface.report_health("active")

    async def stop(self):
        """Stop the CRONOS service gracefully."""
        if not self._running:
            logger.warning("CRONOS service not running.")
            return
        
        logger.info("Stopping CRONOS service...")
        self._running = False
        # Cancel background tasks
        if self._backup_task and not self._backup_task.done():
            self._backup_task.cancel()
            try: await self._backup_task
        ...
        logger.info("CRONOS service stopped.")

    # --- Core Functionality Methods --- #

    async def create_backup(self, name: str, backup_type: str = "manual", metadata: Optional[Dict] = None) -> Optional[str]:
        """Creates a new system backup using the configured logic."""
        logger.info(f"Initiating backup: Name='{name}', Type='{backup_type}'")
        ...
        try:
            ...
            copied_files_count, skipped_files_count, skipped_dirs_count, bytes_copied = await self._execute_backup_file_copy(backup_location)
            ...
            logger.info(f"Backup '{name}' ({backup_id}) completed successfully in {duration:.2f}s.")
            logger.info(f"Stats: {copied_files_count} files copied, {bytes_copied / (1024*1024):.2f} MB")
            ...
            await self.interface.publish_event(
                topic="event.cronos.backup_completed",
                payload=asdict(backup_info)
            )
            return backup_id

        except Exception as e:
            logger.error(f"Backup creation failed for '{name}': {e}", exc_info=True)
            # Attempt cleanup of partial backup directory
            if backup_location.exists():
                try: shutil.rmtree(backup_location)
                except Exception as clean_e: logger.error(f"Failed to clean up partial backup {backup_location}: {clean_e}")
            await self.interface.publish_event(
                 topic="event.cronos.backup_failed",
                 payload={"name": name, "error": str(e)} 
            )
            return None

    async def _execute_backup_file_copy(self, backup_location: Path) -> Tuple[Optional[int], Optional[int], Optional[int], Optional[int]]:
        ...
        Returns:
            Tuple (files_copied, files_skipped, dirs_skipped, bytes_copied) or (None, None, None, None) on failure.
        """
        logger.info(f"Starting file copy process to {backup_location}...")
        files_copied = 0
        files_skipped = 0
        dirs_skipped = 0
        bytes_copied = 0
        ...
        try:
             ...
             logger.warning(f"Backup base path {self.backup_base_path} is outside system root {self.system_root}. Cannot exclude based on prefix.")
        ...
        try:
            ...
            for root, dirs, files in os.walk(self.system_root, topdown=True):
                ...
                if backup_base_rel_path and os.path.relpath(root, self.system_root).startswith(backup_base_rel_path):
                    logger.debug(f"Skipping backup directory itself: {root}")
                ...
                for d in original_dirs:
                    ...
                    if is_excluded:
                         logger.debug(f"Excluding directory: {Path(root) / d}")
                         skipped_in_level += 1
                    else:
                         dirs.append(d)
                ...
                for file in files:
                    ...
                    if src_path.suffix in excluded_exts:
                        logger.debug(f"Skipping file by extension: {src_path}")
                        files_skipped += 1
                        continue
                    ...
                    try:
                        ...
                        try:
                             ...
                             logger.warning(f"Source file disappeared during backup: {src_path}")
                             pass # File might be gone, ignore size error but count as copied
                        except OSError as stat_e:
                             logger.warning(f"Could not get size of {src_path}: {stat_e}")
                             pass # Ignore size if stat fails

                    except Exception as copy_e:
                        logger.error(f"Failed to copy {src_path} to {dst_path}: {copy_e}")
            ...
            logger.error(f"Error during file copy process: {e}", exc_info=True)
    ...
    async def clean_old_backups(self):
        ...
        monthly = retention_policy.get("monthly", 3)

        if not all(isinstance(i, int) and i >= 0 for i in [daily, weekly, monthly]):
             logger.error("Invalid retention policy numbers in config. Skipping cleanup.")
             return
        ...
        try:
            for item in self.backup_base_path.iterdir():
                if item.is_dir() and item.name.startswith("system_backup_"):
                    try:
                        timestamp_str = item.name.replace("system_backup_", "")
                        timestamp = datetime.strptime(timestamp_str, "%Y%m%d_%H%M%S")
                        backups_to_scan.append((item, timestamp))
                    except ValueError:
                        logger.warning(f"Could not parse timestamp from backup directory: {item.name}")
        except FileNotFoundError:
            logger.info("Backup directory not found for cleanup.")
            return
        except Exception as e:
            logger.error(f"Error listing backup directories: {e}", exc_info=True)
            return

        # Sort backups by timestamp (newest first)
        backups_to_scan.sort(key=lambda x: x[1], reverse=True)
        ...
        for backup_path, timestamp in backups_to_scan:
            ...
            if backup_path == backups_to_scan[0][0]:
                 keep = True
        ...
        for backup_path, timestamp in backups_to_scan:
            if backup_path not in keep_paths:
                logger.info(f"Deleting old backup (Reason: Retention Policy): {backup_path}")
                try:
                    shutil.rmtree(backup_path)
                    # Remove from in-memory dict as well
                    self.backups.pop(backup_path.name, None)
                    deleted_count += 1
                except Exception as e:
                    logger.error(f"Failed to delete {backup_path}: {e}")

        logger.info(f"Backup cleanup completed. Kept {len(keep_paths)} backups, deleted {deleted_count}.")
    ...
    def _capture_current_state(self, backup_id: str) -> str:
        ...
        logger.debug(f"Capturing system state {state_id} for backup {backup_id}")
        ...
        try:
            ...
            logger.warning(f"Could not capture OS info: {e}")
        ...
        logger.info(f"Captured system state {state_id}")
        return state_id

    def _get_git_commit_hash(self) -> Optional[str]:
        """Attempts to get the current git commit hash."""
        if not shutil.which("git"): # Check if git command exists
             logger.warning("Git command not found, cannot capture commit hash.")
             return None
        try:
            ...
            return result.stdout.strip()
             ...
             logger.warning("Not a git repository, cannot capture commit hash.")
             return None
        except subprocess.CalledProcessError as e:
            logger.error(f"Error getting git commit hash: {e.stderr}")
            return None
        except Exception as e:
            logger.error(f"Unexpected error getting git commit hash: {e}", exc_info=True)
            return None
        ...
        for rel_path_str in config_files_to_hash:
            ...
            if file_path.exists() and file_path.is_file():
                try:
                    ...
                    logger.error(f"Error hashing config file {rel_path_str}: {e}")
                    hashes[rel_path_str] = "Error"
            else:
                logger.warning(f"Config file not found for hashing: {rel_path_str}")
            ...
            for b_data in history.get("backups", []):
                try:
                    ...
                    if backup.location.exists() and backup.location.is_dir():
                         ...
                         logger.warning(f"Backup directory missing for history entry {backup.id}. Ignoring.")
                except Exception as e:
                     logger.error(f"Error loading backup entry {b_data.get('id')}: {e}")
            ...
            for s_data in history.get("states", []):
                 try:
                     ...
                     self.states[state.id] = state
                      ...
                      logger.error(f"Error loading state entry {s_data.get('id')}: {e}")

            logger.info(f"Version history loaded: {len(self.backups)} backups, {len(self.states)} states.")

        except Exception as e:
            logger.error(f"Failed to load or parse version history {self.version_history_file}: {e}", exc_info=True)
    ...
    def _save_version_history(self):
        ...
        logger.debug("Saving version history...")
        history = {
        ...
        try:
            ...
            logger.info(f"Version history saved to {self.version_history_file}")
        except Exception as e:
            logger.error(f"Failed to save version history: {e}", exc_info=True)

    # --- Placeholder methods from cronos_core.py/preservation.py --- #
    # These need proper implementation based on chosen strategy
    
    def _verify_backup_integrity(self, backup_info: SystemBackupInfo) -> bool:
        ...
        """
        logger.info(f"Verifying integrity of backup: {backup_info.id}...")
        backup_path = backup_info.location
        
        # 1. Basic check: Does the directory exist?
        if not backup_path.exists() or not backup_path.is_dir():
            logger.error(f"Integrity check failed: Backup directory not found at {backup_path}")
            return False
            ...
            for root, dirs, files in os.walk(backup_path):
                ...
                for d in dirs:
                     ...
                     if d in problematic_dirs:
                         ...
                         logger.warning(f"Integrity issue: Problematic directory '{d}' found within backup at {rel_path.relative_to(backup_path)}")
            
            if problems_found:
                logger.error(f"Integrity check failed: Found {len(problems_found)} problematic directories inside backup {backup_info.id}.")
                # Optionally, list problems here or return them?
                return False
            else:
                logger.info(f"Integrity check passed for {backup_info.id} (Excluded directory check)." )
                return True
                
        except Exception as e:
             logger.error(f"Error during integrity check for {backup_info.id}: {e}", exc_info=True)
             return False
    ...
    def check_for_stray_backups(self) -> List[str]:
         ...
         """
         logger.info("Scanning for potential stray backup directories...")
         stray_backups = []
         # Define patterns that might indicate a backup folder
         backup_patterns = set(self.config.get("stray_backup_patterns", 
                                    ['backup', 'bkp', '_backup', '_bkp'])) # Add more patterns if needed
             ...
             for item in self.system_root.rglob('*'): # Use rglob for recursive search
                 if item.is_dir():
                     ...
                     if any(pattern in item.name.lower() for pattern in backup_patterns):
                          ...
                          is_managed = False
                          try:
                               if item.relative_to(latest_backup_path) and item.name in current_backup_ids:
                                    is_managed = True 
                          ...
                          if not is_managed:
                               ...
                               logger.warning(f"Potential stray backup directory found: {rel_path}")
                               stray_backups.append(rel_path)
         except Exception as e:
              logger.error(f"Error scanning for stray backups: {e}", exc_info=True)
              
         if not stray_backups:
              logger.info("No potential stray backup directories found.")
    ...
    def _restore_system_state(self, state_id: str) -> bool:
        ...
        logger.error(f"System state restore for {state_id} not implemented.")
        return False
    ...
    async def _check_and_perform_backup(self):
        ...
        if not sorted_backups:
            ...
            try:
                ...
                # last_backup_timestamp = last_backup_timestamp.replace(tzinfo=datetime.timezone.utc)
                ...
                logger.info(f"Time since last backup: {hours_since_last:.1f} hours.")
                if hours_since_last > interval_hours:
                    backup_needed = True
                    reason = f"Automatic backup (interval: {interval_hours}h, last: {hours_since_last:.1f}h ago)"
                else:
                     logger.info(f"Recent backup found ({hours_since_last:.1f}h ago). No automatic backup needed now.")
                     
            except Exception as e:
                 logger.warning(f"Could not reliably determine time since last backup: {e}. Performing backup as precaution.")
                 backup_needed = True
                 reason = "Precautionary backup (error determining last backup time)"

        if backup_needed:
            logger.info(f"Performing backup. Reason: {reason}")
    ...
    async def handle_list_backups_request(self, message: Dict[str, Any]):
        ...
        logger.info(f"Received list_backups request: {request_id}")
        ...
        try:
            ...
            await self.interface.publish(response_topic, {"type": "list_backups_response", "payload": response_payload})
            logger.info(f"Processed list_backups request {request_id}. Found {len(backup_list)} backups.")
        except Exception as e:
            logger.error(f"Error handling list_backups request {request_id}: {e}", exc_info=True)
            await self.interface.publish(response_topic, {"type": "error", "payload": {"message": str(e)}})

    async def handle_restore_backup_request(self, message: Dict[str, Any]):
        """Handles requests to restore a specific backup."""
        request_id = message.get("id", "unknown")
        logger.info(f"Received restore_backup request: {request_id}")
        response_topic = f"response.{self.node_id}.{request_id}"
        try:
            ...
            if strategy != "new_location" and not restore_target_path:
                 raise ValueError("'restore_target_path' is required for non-'new_location' strategies.")
                 
            logger.warning(f"Initiating restore for backup '{backup_identifier}' with strategy '{strategy}'.")
            ...
            await self.interface.publish(response_topic, {"type": "restore_backup_response", "payload": response_payload})
            logger.info(f"Processed restore_backup request {request_id}. Success: {success}")

        except Exception as e:
            logger.error(f"Error handling restore_backup request {request_id}: {e}", exc_info=True)
            await self.interface.publish(response_topic, {"type": "error", "payload": {"message": str(e)}})
...
def initialize_cronos(mycelium_network_instance: MyceliumNetwork) -> Optional[CronosService]:
    ...
    logger.info("Attempting to initialize CRONOS...")
    try:
        ...
        if config_path.exists():
            ...
            logger.warning(f"CRONOS config file not found at {config_path}. Using defaults.")
        ...
        # await service.start()
        logger.info("CRONOS initialized, awaiting start command.")
        return service
    except Exception as e:
        logger.error(f"CRONOS initialization failed: {e}", exc_info=True)
        return None 