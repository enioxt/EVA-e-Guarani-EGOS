...
class MockMyceliumInterface:
    ...
    async def simulate_message(self, topic: str, payload: dict, sender: str = "mock_sender", msg_type: str = "EVENT", correlation_id: str = None):
...
@pytest.mark.asyncio
async def test_analyze_request_handler(analyzer: NexusAnalyzer, mock_mycelium_interface: MockMyceliumInterface):
    ...
    assert result_event["header"]["topic"] == analyzer.topics["analyze_result"], "Event topic mismatch"
    assert result_event["payload"]["result"]["module"] == module, "Module name mismatch in result"
    assert result_event["payload"]["result"]["dependencies"] == dependencies, "Dependencies mismatch in result"
    assert result_event["payload"]["result"]["metadata"]["version"] == "1.0", "Metadata mismatch in result"
    assert result_event["payload"]["request_id"] == request_corr_id, "Correlation ID mismatch in result event"

@pytest.mark.asyncio
async def test_dependency_update_handler(analyzer: NexusAnalyzer, mock_mycelium_interface: MockMyceliumInterface):
...
@pytest.mark.asyncio
async def test_module_update_handler(analyzer: NexusAnalyzer, mock_mycelium_interface: MockMyceliumInterface):
...
@pytest.mark.asyncio
async def test_error_handling(analyzer: NexusAnalyzer, mock_mycelium_interface: MockMyceliumInterface):
...
@pytest.mark.asyncio
async def test_alert_publishing(analyzer: NexusAnalyzer, mock_mycelium_interface: MockMyceliumInterface):
...
async def test_config_loading(tmp_path):
    ...
    # Check deep merge
    assert analyzer_custom_config.config["mycelium"]["topics"]["analyze_request"] == "custom.request"
    assert analyzer_custom_config.config["mycelium"]["topics"]["analyze_result"] == "nexus.analyze.result" # Default preserved 