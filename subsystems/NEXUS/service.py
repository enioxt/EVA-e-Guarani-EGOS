...
class NexusService:
    ...
    async def start(self):
        ...
        try:
            await self.interface.subscribe(f"request.{self.node_id}.analyze_file", self.handle_analyze_file_request)
            await self.interface.subscribe(f"request.{self.node_id}.analyze_workspace", self.handle_analyze_workspace_request)
            await self.interface.subscribe(f"request.{self.node_id}.suggest_improvements", self.handle_suggest_improvements_request)
    ...
    async def handle_analyze_file_request(self, message: Dict[str, Any]):
        ...
        try:
            ...
            await self.interface.publish(response_topic, {"type": "analyze_file_response", "payload": response_payload})
            self.logger.info(f"Processed analyze_file request {request_id} for '{file_path_str}'. Success: {response_payload['success']}") # Use self.logger

        except Exception as e:
            self.logger.error(f"Error handling analyze_file request {request_id}: {e}", exc_info=True) # Use self.logger
    ...
    async def handle_analyze_workspace_request(self, message: Dict[str, Any]):
        ...
        try:
            ...
            await self.interface.publish(response_topic, {"type": "analyze_workspace_response", "payload": response_payload})
            self.logger.info(f"Processed analyze_workspace request {request_id}. Success: {response_payload['success']}") # Use self.logger

        except Exception as e:
            self.logger.error(f"Error handling analyze_workspace request {request_id}: {e}", exc_info=True) # Use self.logger
            await self.interface.publish(response_topic, {"type": "error", "payload": {"message": str(e)}})
    ...
    async def handle_suggest_improvements_request(self, message: Dict[str, Any]):
        ...
        try:
            ...
            await self.interface.publish(response_topic, {"type": "suggest_improvements_response", "payload": response_payload})
            self.logger.info(f"Processed suggest_improvements request {request_id}. Found {len(suggestions)} suggestions.") # Use self.logger

        except Exception as e:
            self.logger.error(f"Error handling suggest_improvements request {request_id}: {e}", exc_info=True) # Use self.logger
            await self.interface.publish(response_topic, {"type": "error", "payload": {"message": str(e)}})
