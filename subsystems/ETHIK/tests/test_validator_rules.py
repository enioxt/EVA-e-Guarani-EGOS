...
def test_load_rules_from_file():
    ...
    with patch("builtins.open", mock_open(read_data=json.dumps(test_rules_content))), \
...
def test_history_retention():
    ...
    validator = EthikValidator({
        **TEST_CONFIG,
        "validator_config": {"history_retention_days": 1}
    }, mock_interface)
...
def test_rule_severity_threshold():
    ...
    validator = EthikValidator({
        **TEST_CONFIG,
        "validator_config": {"ethical_threshold": 0.9}
    }, mock_interface)
