...
from ..core.validator import EthikValidator, ValidationRule, ValidationResult
...
async def test_handle_action_proposed_error():
    ...
    message = {
        "id": "test-action-3",
        "payload": {} # Empty payload should trigger error handling
    }
...
def test_should_apply_rule():
    ...
    context = {
        "action_type": "file_write",
        "target_path": "/etc/config/test.json"
    }
...
async def test_validation_request_handler(validator, mock_mycelium):
    ...
    validator.validate_action = AsyncMock(return_value=Mock(
        is_valid=True,
        score=0.95,
        details={"passed_rules": ["rule1", "rule2"]},
        severity=0.1
    ))
    
    # Trigger validation request
    await mock_mycelium.trigger_message(
        "test.ethik.validate.request",
        {
            "action": action,
            "context": context,
            "rules": rules
...
async def test_validation_request_failure_alert(validator, mock_mycelium):
    ...
    validator.validate_action = AsyncMock(return_value=Mock(
        is_valid=False,
        score=0.3,
        details={"failed_rules": ["critical_rule"]},
        severity=0.8
    ))
