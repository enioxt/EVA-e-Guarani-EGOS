...
class EthikSanitizer:
    ...
    def __init__(self, config: Dict[str, Any], mycelium_interface: MyceliumInterface, logger: logging.Logger):
    ...
    async def start_monitoring(self):
        ...
        self.logger.info("ETHIK Sanitizer monitoring starting (Mycelium connection assumed active)...") # Use self.logger
        ...
        try:
            ...
            self.logger.error(f"Failed to subscribe to Mycelium topic: {e}", exc_info=True) # Use self.logger
            return
    ...
    async def handle_sanitize_request(self, message: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.info(f"Sanitization complete for request {request_id}. Result published.") # Use self.logger
            
        except Exception as e:
            self.logger.error(f"Error handling sanitization request {request_id}: {e}", exc_info=True) # Use self.logger
            # Send error response
            try:
                await self.interface.publish(
                    topic=f"response.sanitization.{request_id}",
                ...
                self.logger.error(f"Failed to publish error response for {request_id}: {pub_e}") # Use self.logger
    ...
    def sanitize_content(self, content: str, context: Dict[str, Any] = {}) -> SanitizationResult:
        ...
        self.logger.debug(f"Cache miss for content_id: {content_id}. Sanitizing...") # Use self.logger
        ...
        try:
            ...
            for rule_id, rule in self.rules.items():
                if self._should_apply_rule(rule, context):
                    ...
                    if rule_changes:
                        ...
                        severity_penalty = {'critical': 0.5, 'high': 0.2, 'medium': 0.1, 'low': 0.05}
            ...
            self.logger.debug(f"Sanitization finished for content_id: {content_id}. Score: {result.ethical_score:.2f}") # Use self.logger
            return result
            
        except Exception as e:
            self.logger.error(f"Error during sanitization for content_id {content_id}: {e}", exc_info=True) # Use self.logger
    ...
    def _should_apply_rule(self, rule: SanitizationRule, context: Optional[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            allowed_builtins = {"True": True, "False": False, "None": None, "len": len, "in": lambda x, y: x in y}
            ...
            for condition in rule.conditions:
                ...
                if not eval(condition, eval_globals):
                    self.logger.debug(f"Rule {rule.id} condition not met: '{condition}' with context: {context}") # Use self.logger
                    return False
            # If all conditions evaluated to True (or list was empty)
            self.logger.debug(f"Rule {rule.id} conditions met for context: {context}") # Use self.logger
            return True
        except Exception as e:
            self.logger.error(f"Error evaluating rule {rule.id} conditions: {e}. Context: {context}", exc_info=True) # Use self.logger
    ...
    def _apply_rule(self, rule: SanitizationRule, content: str) -> Tuple[str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            self.logger.error(f"Error applying rule {rule.id}: {e}", exc_info=True) # Use self.logger
    ...
    def _create_error_result(self, content_id: str, original_content: str, error_message: str) -> SanitizationResult:
    ...
    def _load_rules(self):
        ...
        if not rules_file_abs_path_str:
            self.logger.error("Absolute path for sanitization rules file not found in configuration.") # Use self.logger
            self.rules.clear()
            return

        rules_path = Path(rules_file_abs_path_str)
        
        self.logger.info(f"Attempting to load sanitization rules from: {rules_path}") # Use self.logger
            ...
            try:
                ...
                for rule_dict in rules_data.get("rules", []):
                     try:
                         ...
                         self.logger.error(f"Error creating SanitizationRule instance for rule ID '{rule_dict.get('id')}': Missing or invalid arguments - {te}", exc_info=True) # Use self.logger
                     except Exception as item_e:
                          self.logger.error(f"Error parsing sanitization rule item {rule_dict.get('id')}: {item_e}", exc_info=True) # Use self.logger
                self.logger.info(f"Loaded {len(self.rules)} sanitization rules from {rules_path.name}") # Use self.logger
            except json.JSONDecodeError:
                self.logger.error(f"Invalid JSON in rules file: {rules_path}") # Use self.logger
            except Exception as e:
                self.logger.error(f"Error loading sanitization rules file {rules_path}: {e}", exc_info=True) # Use self.logger
                self.rules.clear() # Clear rules on error
        else:
            self.logger.warning(f"Sanitization rules file not found: {rules_path}. Sanitizer will have no rules.") # Use self.logger
    ...
    def add_rule(self, rule_dict: Dict[str, Any]):
        ...
        try:
            ...
            self.rules[rule.id] = rule
            self.logger.info(f"Added/Updated sanitization rule: {rule.name} [{rule.id}]") # Use self.logger
            # Consider saving rules back to file if dynamic updates should persist
        except Exception as e:
            self.logger.error(f"Error adding/updating rule {rule_dict.get('id')}: {e}", exc_info=True) # Use self.logger
    ...
    def remove_rule(self, rule_id: str):
        ...
        if rule_id in self.rules:
            ...
            self.logger.info(f"Removed sanitization rule: {removed_rule_name} [{rule_id}]") # Use self.logger
             # Consider saving rules back to file if dynamic updates should persist
        else:
            self.logger.warning(f"Attempted to remove non-existent rule: {rule_id}") # Use self.logger
    ...
    def _clean_history(self):
        ...
        if removed_count > 0:
            self.logger.debug(f"Cleaned {removed_count} old entries from sanitization history.") # Use self.logger
    ...
    def clear_history(self, older_than: Optional[datetime.datetime] = None):
        ...
        if older_than:
            ...
            removed_count = initial_count - len(self.sanitization_history)
            self.logger.info(f"Cleared {removed_count} sanitization history entries older than {older_than}.") # Use self.logger
        else:
            self.sanitization_history = []
            self.logger.info(f"Cleared all {initial_count} sanitization history entries.") # Use self.logger
...
if __name__ == '__main__':
    ...
    mock_config = {
        "sanitizer_config": {
            "rules_file": "../config/sanitization_rules.json" # Adjust if needed
        },
