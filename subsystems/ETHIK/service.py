...
class EthikService:
    ...
    def __init__(self, config: Dict[str, Any], mycelium_interface: MyceliumInterface, project_root: Path):
        ...
        validation_rules_rel_path = validator_config.get("rules_file") or self.config.get("validation_rules_file", "config/validation_rules.json")
        sanitization_rules_rel_path = sanitizer_config.get("rules_file") or self.config.get("sanitization_rules_file", "config/sanitization_rules.json")
