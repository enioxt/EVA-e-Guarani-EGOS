...
def temp_test_env(tmp_path):
    ...
    config = {
        "integration": {
            "bios_q": {"enabled": True, "check_on_startup": True, "block_on_error": False}
        }
    }
...
def test_validate_system_success(bios_integration):
    ...
    mock_results = {"stats": {"violations": {"error": 0, "warning": 2}}}
...
def test_validate_system_failure(bios_integration):
    ...
    mock_results = {"stats": {"violations": {"error": 2, "warning": 1}}}
...
def test_initialize_validation_success():
    ...
    with (
        patch.dict(os.environ, {"EVA_GUARANI_ROOT": "/test/path"}),
        patch("pathlib.Path.exists", return_value=True),
        patch("builtins.open", MagicMock()),
        patch("json.dump", MagicMock()),
