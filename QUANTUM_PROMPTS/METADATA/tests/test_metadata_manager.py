...
def test_metadata_manager_initialization(metadata_manager):
    ...
    assert metadata_manager.supported_encodings == [
...
def test_file_encoding_handling(metadata_manager, temp_dir):
    ...
    for encoding, content in encodings.items():
        ...
        with open(file_path, 'r', encoding=encoding) as f:
            ...
            assert json.loads(processed_content.split("'''\n")[1].split("\n'''")[0].replace("METADATA:\n", ""))
...
def test_subsystem_detection(metadata_manager, temp_dir):
    ...
    for subsystem in subsystems:
        # Create a file in each subsystem directory
        subsystem_dir = Path(temp_dir) / subsystem
        subsystem_dir.mkdir(exist_ok=True)
        file_path = subsystem_dir / "test.py"
