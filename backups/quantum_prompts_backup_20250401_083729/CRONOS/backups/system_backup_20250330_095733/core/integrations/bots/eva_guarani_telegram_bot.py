...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            os.path.join(current_dir, "logs", "telegram_bot.log"), encoding="utf-8"
        ),
...
class EVAGuaraniBot:
    ...
    def __init__(self, config_path: str = os.path.join(current_dir, "telegram_config.json")):
        ...
        if not TELEGRAM_AVAILABLE:
            self.logger.error(
                "python-telegram-bot is required. Run: pip install python-telegram-bot"
            )
    ...
    async def initialize_quantum_knowledge(self) -> bool:
        ...
        try:
            ...
            integrator_config_path = self.config.get("knowledge", {}).get(
    ...
    def setup_handlers(self) -> None:
        ...
        self.dispatcher.add_handler(
            MessageHandler(
                filters.Text(~filters.Command) & ~filters.Regex(r"/cancel"), self._handle_message
            )
        )
    ...
    def _handle_restart(self, update: Update, context: ContextTypes) -> None:
        ...
        # Send confirmation
        update.message.reply_text("✅ Conversation context has been reset.")
    ...
    def _handle_upgrade(self, update: Update, context: ContextTypes) -> None:
        ...
        credits_per_payment = self.payment_gateway.config.get("pricing", {}).get(
        ...
        btc_address = (
            self.payment_gateway.config.get("crypto", {}).get("btc", {}).get("address", "")
        )
        sol_address = (
            self.payment_gateway.config.get("crypto", {}).get("sol", {}).get("address", "")
        )
        eth_address = (
            self.payment_gateway.config.get("crypto", {}).get("eth", {}).get("address", "")
        )
        ...
        keyboard = [[InlineKeyboardButton("💰 Register Payment", callback_data="register_payment")]]
    ...
    def _handle_image_prompt(self, update: Update, context: ContextTypes) -> int:
        ...
        try:
            ...
            if self.payment_gateway and PAYMENT_AVAILABLE:
                ...
                image_cost = (
                    self.payment_gateway.config.get("pricing", {})
                    .get("image_generation_cost", {})
                    .get("standard", 2)
                )
            ...
            self.add_to_conversation_history(
                chat_id, "assistant", "I've generated an image based on your prompt."
            )
        ...
        if OPENAI_AVAILABLE:
            try:
                ...
                response = openai.ChatCompletion.create(
                    model="gpt-3.5-turbo", messages=formatted_history
                )
    ...
    def add_to_conversation_history(self, chat_id: int, role: str, message: str) -> None:
        ...
        self.conversations[chat_id].append(
            {"role": role, "content": message, "timestamp": datetime.datetime.now().isoformat()}
        )
