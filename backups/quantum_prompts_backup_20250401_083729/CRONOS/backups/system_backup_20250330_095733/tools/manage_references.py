...
def main():
    parser = argparse.ArgumentParser(
        description="Manage references to large directories in EVA & GUARANI project"
    )
    ...
    move_parser.add_argument(
        "--no-link", action="store_true", help="Don't create a symbolic link after moving"
    )
    ...
    setup_parser.add_argument(
        "--refs-dir", default="../EVA_REFS", help="Directory to store references"
    )
        ...
        try:
            ...
            for dir_name in large_dirs:
                if Path(dir_name).exists():
                    ...
                    print(
                        f"python tools/manage_references.py move {dir_name} {refs_dir}/{dir_type}"
                    )
