...
# Page configuration
st.set_page_config(page_title="EVA & GUARANI - ATLAS", page_icon="🌌", layout="wide")
...
def create_node_form():
    ...
    with st.form("node_form"):
        ...
        node_type = st.selectbox(
            "Node Type", options=["System", "Component", "Concept", "Entity", "Process"]
        )
        ...
        if submitted and name:
            if st.session_state.atlas_initialized:
                try:
                    metadata = {"description": description, "created_by": "EVA_Frontend"}
        ...
        if submitted and source_name != target_name:
            if st.session_state.atlas_initialized:
                try:
                    ...
                    if conn_id:
                        st.success(
                            f"Connection created successfully between '{source_name}' and '{target_name}'!"
                        )
    ...
    if st.session_state.atlas_initialized:
        try:
            ...
            with st.expander("Analysis Details"):
                ...
                st.markdown(
                    f"**Average Consciousness Level:** {analysis.get('average_consciousness_level', 0):.4f}"
                )
