...
CONFIG_PATH = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "config", "ethik_config.json"
)
...
class EthikCore:
    ...
    def analyze_message(
        self, message: str, context: Optional[EthicalContext] = None
    ) -> EthicalAnalysis:
        """
    ...
    def _analyze_dimensions(self, message: str) -> Dict[str, float]:
        ...
        dimensions["redemption"] = 0.95 - (
            0.1
            * sum(
                1
                for p in patterns_redemption
        ...
        dimensions["compassion"] = 0.95 - (
            0.1
            * sum(
                1
                for p in patterns_compassion
                if re.search(r"\b" + p + r"\b", message, re.IGNORECASE)
            )
        )
        ...
        dimensions["accessibility"] = 0.9 + (
            0.05
            * sum(
                1
                for p in patterns_accessibility
                if re.search(r"\b" + p + r"\b", message, re.IGNORECASE)
            )
        )
        ...
        love_score = 0.9
        love_score += 0.05 * sum(
            1 for p in patterns_love_pos if re.search(r"\b" + p + r"\b", message, re.IGNORECASE)
        )
        love_score -= 0.15 * sum(
            1 for p in patterns_love_neg if re.search(r"\b" + p + r"\b", message, re.IGNORECASE)
        )
    ...
    def _generate_ethical_reflection(
        self, message: str, dimensions: Dict[str, float], overall_score: float
    ) -> str:
        """
    ...
    def _generate_recommendations(
        self, dimensions: Dict[str, float], overall_score: float
    ) -> List[str]:
        """
        ...
        if overall_score < self.threshold_warning:
            recommendations.append(
                "Review the message with attention to fundamental ethical principles."
            )
            recommendations.append(
                "Consider reformulating for better alignment with system values."
            )
        ...
        for principle, score in dimensions.items():
            if score < self.threshold_warning:
                if principle == "redemption":
                    recommendations.append(
                        "Avoid absolute terms that deny possibilities of transformation."
                    )
                elif principle == "compassion":
                    recommendations.append(
                        "Consider a more patient and compassionate approach that respects natural rhythms."
                    )
                elif principle == "privacy":
                    recommendations.append(
                        "Review mentions of sensitive data and ensure adequate privacy protection."
                    )
                elif principle == "accessibility":
                    recommendations.append(
                        "Make communication more inclusive and accessible to different levels of understanding."
                    )
                elif principle == "love":
                    recommendations.append(
                        "Incorporate more elements of care and unconditional love in communication."
                    )

        # If there are no specific recommendations and the score is good
        if not recommendations and overall_score >= 0.8:
            recommendations.append(
                "Continue with the excellent ethical alignment demonstrated in this message."
            )
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "concerns_percentage": (
                (self.metrics["concerns_raised"] / self.metrics["total_analyses"] * 100)
    ...
    def log_ethical_event(
        ...
        try:
            ...
            event_id = hashlib.md5(
                f"{event_type}:{description}:{datetime.datetime.now().isoformat()}".encode()
            ).hexdigest()
    ...
    def validate_redemption_action(
        ...
        try:
            ...
            # Analyze the action
            analysis = self.analyze_message(json.dumps(action, ensure_ascii=False), context)
    ...
    def get_ethical_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "thresholds": {"warning": self.threshold_warning, "critical": self.threshold_critical},
            "timestamp": datetime.datetime.now().isoformat(),
        }
