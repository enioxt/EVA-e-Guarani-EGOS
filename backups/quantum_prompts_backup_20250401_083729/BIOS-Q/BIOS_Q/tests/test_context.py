...
def test_context_system():
    ...
    context_files = [f for f in os.listdir(context_dir) if f.startswith("bios_q_context_") and f.endswith(".json")]
    ...
    test_file = os.path.join(context_dir, f"test_context_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
    try:
    ...
    backup_dir = os.path.join(context_dir, "backup_context_" + datetime.now().strftime("%Y%m%d_%H%M%S"))
