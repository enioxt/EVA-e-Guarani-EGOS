    ...
    try:
        ...
        try:
            ...
            def patched_astimezone(obj):
                ...
                # If it's already a pytz timezone, return directly
                    ...
                    if hasattr(app, "job_queue") and app.job_queue is not None:
                        ...
                        def app_start_wrapper():
                            ...
                            try:
                                ...
                                if (
                                    hasattr(app.job_queue, "_scheduler")
                                    and app.job_queue._scheduler is not None
                                ):
                                    app.job_queue._scheduler.timezone = pytz.UTC
                                    print(
                                        "✅ Scheduler timezone set to pytz.UTC after initialization"
                                    )
                            except Exception as e:
                        ...
                        # Replace the start method only on the app object, not the class
...
try:
    ...
    print(f"✅ Pytz imported successfully: version {getattr(pytz, '__version__', 'unknown')}")
    ...
    try:
        ...
        if hasattr(jobqueue_module, "JobQueue"):
            ...
            if hasattr(JobQueue, "_create_scheduler"):
                ...
                print("✅ Patch applied to JobQueue's _create_scheduler method")

            # If we can't modify _create_scheduler, try another entry point
            if hasattr(AsyncIOScheduler, "__init__"):
                ...
                print("✅ Decorator applied to ensure pytz timezone in AsyncIOScheduler")
