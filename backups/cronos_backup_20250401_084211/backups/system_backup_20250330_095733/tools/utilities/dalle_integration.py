...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/dalle_integration.log"), logging.StreamHandler()],
)
...
class DalleIntegration:
        ...
        try:
            if os.path.exists(self.config_path):
                ...
                default_config = {
                    ...
                    "default_style": "vivid",
                    "rate_limit": {"requests_per_minute": 50, "requests_per_day": 1000},
    ...
    def generate_image(
        ...
        if size not in valid_sizes[model]:
            return (
                False,
                f"Invalid size for {model}: {size}. Valid sizes: {', '.join(valid_sizes[model])}",
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"Images generated successfully. Prompt: '{prompt[:50]}...' Model: {model}"
                )
    ...
    def get_image_cost(self, model: str, size: str, quality: str = "standard") -> int:
        ...
        # Cost table (credits)
        costs = {
            "dall-e-2": {"256x256": 1, "512x512": 1, "1024x1024": 2},
            "dall-e-3": {
                "1024x1024": {"standard": 2, "hd": 3},
                "1792x1024": {"standard": 3, "hd": 4},
                "1024x1792": {"standard": 3, "hd": 4},
            },
        }
