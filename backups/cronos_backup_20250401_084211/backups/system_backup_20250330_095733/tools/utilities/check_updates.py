...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/update_checker.log"), logging.StreamHandler()],
)
...
def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force check even if it was recently checked"
    )
    parser.add_argument(
        "--interval",
        type=int,
        default=24,
        help="Minimum interval between checks in hours (default: 24)",
    )
    parser.add_argument(
        "--notify-always", action="store_true", help="Notify even if there are no updates"
    )
    ...
    if not args.force:
        ...
        if last_check is not None:
            ...
            if time_since_last_check < timedelta(hours=args.interval):
                logger.info(
                    f"Last check was {time_since_last_check.total_seconds() / 3600:.1f} hours ago."
                )
