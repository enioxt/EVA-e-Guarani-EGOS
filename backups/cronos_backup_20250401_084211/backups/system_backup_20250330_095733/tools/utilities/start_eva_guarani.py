...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/startup.log"), logging.StreamHandler()],
)
...
def check_dependencies():
    ...
    required_packages = ["python-telegram-bot", "openai", "pillow", "requests", "psutil"]
        ...
        if detached:
            ...
            if sys.platform == "win32":
                ...
                process = subprocess.Popen(
                    f'start "EVA & GUARANI Bot" /min {python_exe} {bot_script_path}', shell=True
                )
...
def main():
    ...
    parser.add_argument(
        "--restart", action="store_true", help="Restart the bot if it is already running"
    )
