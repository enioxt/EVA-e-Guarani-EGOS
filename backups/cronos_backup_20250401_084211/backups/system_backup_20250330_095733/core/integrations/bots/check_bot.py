            ...
            try:
                ...
                if name == "Telegram Config":
                    if "token" in config and config["token"]:
                        print(f"  ✅ {name}: Valid (Token: {config['token'][:6]}...{config['token'][-4:]})")
                    else:
    ...
    try:
        ...
        if usage_limits:
            ...
            for tier, limits in usage_limits.items():
                print(f"     • {tier}: {limits.get('messages_per_day', 0)} msgs/day, {limits.get('special_calls_per_day', 0)} special calls/day")
        else:
