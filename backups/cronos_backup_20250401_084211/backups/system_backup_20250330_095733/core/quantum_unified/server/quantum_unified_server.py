...
class QuantumUnifiedServer:
    ...
    async def register(self, websocket: websockets.WebSocketServerProtocol, subsystem: str):
        ...
        if subsystem in self.clients:
            ...
            logger.info(
                f"Client connected to {subsystem}. Total clients: {len(self.clients[subsystem])}"
            )
    ...
    async def unregister(self, websocket: websockets.WebSocketServerProtocol, subsystem: str):
        ...
        if subsystem in self.clients and websocket in self.clients[subsystem]:
            ...
            logger.info(
                f"Client disconnected from {subsystem}. Total clients: {len(self.clients[subsystem])}"
            )
    ...
    async def send_subsystem_state(
        self, websocket: websockets.WebSocketServerProtocol, subsystem: str
    ):
    ...
    async def handle_message(
        self, websocket: websockets.WebSocketServerProtocol, subsystem: str, message: str
    ):
