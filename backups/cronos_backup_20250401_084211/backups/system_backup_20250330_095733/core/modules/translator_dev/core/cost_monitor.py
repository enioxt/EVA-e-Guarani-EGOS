...
class CostMonitor:
    ...
    def record_usage(self, model: str, input_tokens: int, output_tokens: int) -> float:
        ...
        model_base_price = self.config.models_pricing.get(
            model, self.config.models_pricing.get("gpt-3.5-turbo", 0.001)
        )
        ...
        if current_month not in self.usage_data["months"]:
            self.usage_data["months"][current_month] = {"cost": 0.0, "tokens": 0, "models": {}}
        ...
        if model not in month_data["models"]:
            month_data["models"][model] = {"cost": 0.0, "tokens": 0, "calls": 0}
    ...
    def get_current_usage(self) -> Dict[str, Any]:
        ...
        usage = {
            ...
            "percent_used": (
                month_data.get("cost", 0.0) / self.config.monthly_budget * 100
    ...
    def should_use_paid_api(self, estimated_tokens: int, model: str) -> Tuple[bool, str]:
        ...
        new_percent = (
            new_total / self.config.monthly_budget * 100 if self.config.monthly_budget > 0 else 0
        )

        if (
            new_percent >= self.config.warn_at_percent
            and current_usage["percent_used"] < self.config.warn_at_percent
        ):
            logger.warning(
