...
class OpenAITranslator:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "settings") and config.settings.get("terminology", {}).get(
            "enabled", True
        ):
            try:
    ...
    def _initialize_api(self) -> None:
        ...
        try:
            ...
            if not api_key:
                self.logger.warning(
                    "No OpenAI API key found. Please set OPENAI_API_KEY environment variable or configure it in settings."
                )
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a professional translator. Translate the following Portuguese text to English. Maintain the same formatting and style. Keep technical terms intact.",
                    },
            ...
            if hasattr(response, "usage") and hasattr(response.usage, "total_tokens"):
                ...
                if self.budget > 0 and self.cost_spent > self.budget:
                    self.logger.warning(
                        f"Budget exceeded: ${self.cost_spent:.2f} spent (limit: ${self.budget:.2f})"
                    )
    ...
    def translate_file_content(
        self, content: str, file_path: Optional[str] = None, file_ext: Optional[str] = None
    ) -> str:
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a professional translator specialized in Markdown documents. Translate the following Portuguese text to English. Carefully preserve all Markdown formatting, links, code blocks, headers, and other syntax elements. Do not translate text inside code blocks or code spans.",
                    },
    ...
    def _translate_html(self, content: str) -> str:
        ...
        try:
            ...
            self.logger.warning(
                "BeautifulSoup not installed, falling back to basic HTML translation"
            )
            ...
            if format_type == "json":
                ...
                response = self.client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": f"You are a professional translator specialized in {format_type.upper()} documents. Translate the following Portuguese text to English. Carefully preserve all {format_type.upper()} structure, indentation, and syntax. Only translate string values, not keys or structural elements.",
                        },
                        {"role": "user", "content": content},
                    ],
                    temperature=self.temperature,
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": f"You are a professional translator specialized in {language} code. Translate ONLY the comments from Portuguese to English. Do not modify any code, variable names, function names, or other code elements. Preserve the exact structure, indentation, and formatting of the original code.",
                    },
                    {"role": "user", "content": content},
                ],
                temperature=self.temperature,
