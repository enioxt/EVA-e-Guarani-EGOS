...
class QuantumApprovalUI:
    ...
    def __init__(self, root):
        ...
        self.style.configure('Quantum.TLabel', font=('Arial', 12), foreground='#6200ea', background='#f0f0f0')
    ...
    def create_ui(self):
        ...
        ttk.Label(title_frame, text="Quantum Approval System", style='Quantum.TLabel').pack(side=tk.LEFT, padx=10)
        ...
        ttk.Button(button_frame, text="Aprovar Selecionadas", command=self.approve_selected).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Rejeitar Selecionadas", command=self.reject_selected).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Criar Proposta BIOS-Q", command=self.create_biosq_proposal).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Criar Backup", command=self.create_backup).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Atualizar", command=self.reload_data).pack(side=tk.RIGHT, padx=5)
        ...
        ttk.Label(info_details, text="Importância:").grid(row=2, column=0, sticky='w', padx=5, pady=2)
        ...
        ttk.Button(action_frame, text="Aprovar", command=self.approve_current, style='Accept.TButton').pack(side=tk.LEFT, padx=5)
        ttk.Button(action_frame, text="Rejeitar", command=self.reject_current, style='Reject.TButton').pack(side=tk.LEFT, padx=5)
    ...
    def populate_entry_list(self):
        ...
        for entry in self.entries:
            ...
            item_id = self.entry_tree.insert('', 'end', values=(entry_id, category, importance, source))
    ...
    def create_biosq_proposal(self):
        ...
        try:
            ...
            if not os.path.exists(BIOS_CONFIG):
                messagebox.showerror("Erro", f"Arquivo de configuração da BIOS-Q não encontrado: {BIOS_CONFIG}")
                return
            ...
            if 'metrics' in bios_config:
                ...
                importance_sum = sum(entry.get('importance', 0) for cat in entries_by_category 
            ...
            if messagebox.askyesno("Proposta Criada", 
    ...
    def create_backup(self):
        ...
        try:
            ...
            if os.path.exists(QUANTUM_PROMPT):
                ...
                with open(QUANTUM_PROMPT, 'r', encoding='utf-8') as src, \
            ...
            with open(CHANGELOG_FILE, 'r', encoding='utf-8') as src, \
