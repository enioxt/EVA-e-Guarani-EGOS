...
def check_python_version():
    ...
    if current_version < required_version:
        ...
        print(
            f"       Python {required_version[0]}.{required_version[1]} or higher is required.{Style.RESET_ALL}"
        )
        return False
    else:
...
def main():
    ...
    print(
        f"{Fore.CYAN}System: {platform.system()} {platform.release()} ({platform.architecture()[0]}){Style.RESET_ALL}"
    )
    ...
    if missing_deps:
        ...
        install = (
            input("Do you want to install the missing mandatory dependencies? (y/n): ").lower()
            == "y"
        )
        ...
        if install:
            for dep in missing_deps:
                ...
                if success:
                    print(
                        f"{Fore.GREEN}[OK] {dep['name']} installed successfully!{Style.RESET_ALL}"
                    )
                else:
    ...
    if missing_optional_deps:
        ...
        install = (
            input("Do you want to install the missing optional dependencies? (y/n): ").lower()
            == "y"
        )
