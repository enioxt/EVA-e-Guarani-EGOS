...
try:
    ...
    raise ImportError("The 'openai' library is not installed. Run 'pip install openai' to install it.")
...
class PerplexityService:
    ...
    def search(self, query: str, validate_level: str = "normal", model: Optional[str] = None) -> Dict[str, Any]:
        ...
        """
        if not self.client:
            raise RuntimeError("Perplexity service not initialized. Check if the API key is configured.")
        ...
        try:
            ...
            if "401" in error_msg:
                raise RuntimeError(f"Authentication error: The Perplexity API key is invalid or expired. {error_msg}")
    ...
    def _try_models_in_order(self, query: str, validate_level: str) -> Dict[str, Any]:
        ...
        for model in models_to_try:
            try:
                ...
                if "401" in error_msg:
                    raise RuntimeError(f"Authentication error: The Perplexity API key is invalid or expired.")
