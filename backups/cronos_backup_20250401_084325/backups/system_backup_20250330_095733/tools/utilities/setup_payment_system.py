...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"logs/setup_payment_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log", encoding="utf-8"),
...
def create_config_file():
    ...
    try:
        ...
        if os.path.exists(config_path):
            ...
            response = input("The configuration file already exists. Do you want to overwrite it? (y/N): ")
            if response.lower() != 'y':
...
def create_payments_file():
    ...
    try:
        ...
        if os.path.exists(payments_path):
            ...
            # Ask if you want to overwrite
            response = input("The payments file already exists. Do you want to overwrite it? (y/N): ")
            if response.lower() != 'y':
