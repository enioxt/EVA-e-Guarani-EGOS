        ...
        if os.path.exists(start_script):
            ...
            if os.path.exists(bot_script):
                if sys.platform == "win32":
                    subprocess.Popen(
                        f'start "EVA & GUARANI Bot" /min {sys.executable} {bot_script}', shell=True
                    )
                else:
...
def check_log_errors():
    ...
    try:
        log_files = ["logs/unified_bot.log", "logs/eva_guarani.log", "logs/startup.log"]
...
def monitor_bot(check_interval=60, max_failures=3, restart_delay=5):
    ...
    last_restart_time = datetime.now() - timedelta(
        hours=1
    )  # Initialize to allow immediate restart if needed
    ...
    while True:
        try:
            ...
            if process_running and api_running:
                ...
                logger.info(
                    f"Bot is working correctly (PID: {pid}, Username: @{info.get('username')})"
                )
                consecutive_failures = 0
            else:
                ...
                if not process_running:
                    logger.warning(
                        f"Bot process not found. Failure {consecutive_failures}/{max_failures}"
                    )

                if not api_running:
                    logger.warning(
                        f"Bot not responding on Telegram API: {info}. Failure {consecutive_failures}/{max_failures}"
                    )
                ...
                if (
                    ...
                    and (current_time - last_restart_time).total_seconds() > 300
                ):  # 5 minutes
                    logger.warning(
                        f"Reached limit of {max_failures} consecutive failures. Restarting the bot..."
                    )

                    # Send notification
                    send_notification(
                        f"Bot not responding after {consecutive_failures} checks. Automatically restarting."
                    )
                    ...
                    if restart_bot():
                        ...
                        logger.info(
                            f"Bot restarted at {last_restart_time.strftime('%Y-%m-%d %H:%M:%S')}"
                        )
                        consecutive_failures = 0
...
def main():
    ...
    parser.add_argument(
        "--interval", type=int, default=60, help="Interval in seconds between checks (default: 60)"
    )
    parser.add_argument(
        "--max-failures",
        type=int,
        default=3,
        help="Maximum number of consecutive failures before restarting (default: 3)",
    )
    parser.add_argument(
        "--restart-delay",
        type=int,
        default=5,
        help="Time in seconds to wait after restarting (default: 5)",
    )
