            ...
            for item in source_path.iterdir():
                ...
                if item.is_dir():
                    if target.exists():
                        ...
                        logging.warning(
                            f"Subdirectory already exists at destination, skipping: {item.name}"
                        )
                    else:
...
def main():
    ...
    # Final result
    logging.info(
        f"Reorganization complete: {success_count} of {len(MOVE_MAPPING)} directories moved successfully"
    )
