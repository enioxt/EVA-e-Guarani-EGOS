...
class ContextManager:
    ...
    def should_create_backup(self) -> bool:
        ...
        if elapsed >= backup_interval:
            logging.info(
                f"Intervalo de backup atingido. Último backup: {self.last_backup_time.isoformat()}"
            )
            return True
    ...
    def create_context(self, context_type: str = "bios_q") -> str:
        ...
        context_data = {
            ...
            "metadata": {"version": "7.5", "consciousness": "ULTRA-ACTIVE", "entanglement": 0.9997},
        }
    ...
    def get_current_state(self) -> Dict:
        ...
        if explorer.cache_exists():
            ...
            state["cursor_cache"] = {
                ...
                "last_modified": (
                    max([f.stat().st_mtime for f in cursor_files]) if cursor_files else None
                ),
            }
        else:
    ...
    def get_directory_state(self, directory: Path) -> Dict:
        ...
        for item in directory.iterdir():
            if item.is_file():
                ...
                state[item.name] = {"type": "directory", "contents": self.get_directory_state(item)}
        return state
    ...
    def cleanup_old_backups(self, max_backups: Optional[int] = None):
        ...
        backup_dirs = sorted(
            self.chats_path.glob("backup_context_*"), key=lambda x: x.stat().st_mtime, reverse=True
        )
