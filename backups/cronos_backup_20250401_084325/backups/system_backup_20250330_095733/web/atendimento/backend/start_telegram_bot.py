...
if not os.path.exists(env_path):
    ...
    if os.path.exists(env_example_path):
        ...
        logger.error(
            f"Arquivo .env.example não encontrado em {env_example_path}. Crie um arquivo .env manualmente."
        )
        sys.exit(1)
...
try:
    ...
    logger.error(
        "Verifique se as dependências estão instaladas. Execute: pip install -r requirements.txt"
    )
    sys.exit(1)
