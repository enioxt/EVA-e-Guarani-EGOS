...
def save_profile(profile_name, enabled_modules):
    ...
    profile_data = {"name": profile_name, "enabled_modules": enabled_modules}
    ...
    try:
        ...
        for item in modules_path.glob("*"):
            if (
                ...
                and not item.name.endswith(".disabled")
            ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Manage EVA & GUARANI modules to improve performance"
    )
    ...
    group_parser.add_argument(
        "action", choices=["list", "enable", "disable"], help="Action to perform on the group"
    )
    ...
    profile_parser.add_argument(
        "action", choices=["list", "save", "load"], help="Action to perform with profiles"
    )
