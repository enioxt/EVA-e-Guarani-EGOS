...
# Page configuration
st.set_page_config(page_title="EVA & GUARANI - CRONOS", page_icon="⌛", layout="wide")
...
def create_backup_form():
    ...
    with st.form("backup_form"):
        ...
        backup_type = st.selectbox(
            "Backup Type", options=["Complete", "Incremental", "Differential"]
        )
            ...
            if st.session_state.cronos_initialized:
                try:
                    ...
                    backup_id = st.session_state.eva_system.cronos.create_backup(
                        name=backup_name, backup_type=backup_type, metadata=metadata
                    )
...
def restore_backup_form():
    ...
    with st.form("restore_form"):
        ...
        selected_backup = st.selectbox("Select Backup", options=list(backup_options.keys()))
        ...
        if submitted and selected_backup:
            if st.session_state.cronos_initialized:
                try:
                    ...
                    # Confirm restoration
                    if st.warning(
                        "⚠️ This operation will restore the system to a previous state. Do you want to continue?"
                    ):
...
def create_evolution_timeline():
    ...
    fig.update_layout(xaxis_title="Date", yaxis_title="State Type", showlegend=True)
