...
class StatePreservationManager:
    ...
    def _load_state_index(self) -> Dict[str, Any]:
        ...
        if not self.index_file.exists():
            return {"states": [], "last_updated": format_timestamp(), "version": "7.5"}
    ...
    def save_system_state(self, state_name: str, description: str = "") -> str:
        ...
        preserved_files = []
        preserved_stats = {"total_files": 0, "preserved_files": 0, "total_size": 0}
    ...
    def restore_system_state(self, state_id: str) -> bool:
        ...
        logger.info(
            f"Restored {restored_files} files from state '{state_info['name']}' ({state_id})"
        )
        return True
    ...
    def delete_system_state(self, state_id: str) -> bool:
        ...
        # Find state in index
        state_index = next(
            (i for i, s in enumerate(self.state_index["states"]) if s["id"] == state_id), -1
        )
        if state_index == -1:
            logger.error(f"State with ID {state_id} not found")
            return False
    ...
    def analyze_changes(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error loading state metadata: {e}")
            return {"changes_detected": False, "message": f"Error loading state metadata: {e}"}
...
def analyze_changes_cmd(config: Config) -> None:
    ...
    if compared_with:
        ...
        try:
            timestamp = datetime.fromisoformat(compared_with.get("timestamp")).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
