...
def mock_config():
    return {
        ...
        "subsystems": {"ATLAS": {"required": True}, "CRONOS": {"required": True}},
        "initialization": {"display_banner": True},
    }
...
async def test_process_message_error(mcp, mock_writer):
    ...
    # Assert
    expected_error = {"type": "error", "id": "123", "error": "Test error"}
...
async def test_run_success(mcp):
    ...
    with (
        patch.object(mcp, "initialize", side_effect=mock_initialize),
...
async def test_main_initialization():
    ...
    with (
        patch("mcp.bios_q_mcp.BiosQMCP", return_value=mock_mcp),
