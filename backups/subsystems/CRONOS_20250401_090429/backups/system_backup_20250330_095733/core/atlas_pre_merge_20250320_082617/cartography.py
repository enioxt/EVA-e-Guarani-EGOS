...
class Atlas:
    ...
    def __init__(self, config: Dict[str, Any], system_root: Path):
        ...
        self.logger.info(f"ATLAS initialized with depth {self.mapping_depth} and threshold {self.connection_threshold}")
    ...
    def _initialize_mapping(self) -> None:
        ...
        self.system_map = {
            "nodes": {
                ...
                "modules": {
                    "type": "category",
                    "children": ["quantum", "analysis", "integration"]
                },
                "integrations": {
                    "type": "category",
                    "children": ["obsidian", "api", "bots"]
                },
                "tools": {
                    "type": "category",
                ...
                "data": {
                    "type": "category",
                    "children": ["logs", "personas", "examples"]
                }
            }
        }
        ...
        self.logger.info(f"Mapping initialized with {len(self.system_map['nodes'])} main categories")
    ...
    def _generate_connections(self) -> None:
        ...
        self.connections = [
            ...
            {"source": "nexus", "target": "modules/analysis", "strength": 0.9, "type": "dependency"},
            {"source": "cronos", "target": "data/logs", "strength": 0.75, "type": "data_flow"},
            {"source": "ethik", "target": "modules/integration", "strength": 0.6, "type": "influence"},
            {"source": "integrations/obsidian", "target": "tools/visualization", "strength": 0.85, "type": "utilization"}
        ]
    ...
    def generate_visualization(self, format: str = "mermaid") -> str:
        ...
        if format == "mermaid":
            ...
            return json.dumps({"system_map": self.system_map, "connections": self.connections}, indent=2)
        else:
    ...
    def analyze_component(self, component_path: str) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "strength": {
                "incoming": sum([c["strength"] for c in self.connections if c["target"] == component_path]),
                "outgoing": sum([c["strength"] for c in self.connections if c["source"] == component_path])
            },
            "complexity": 0.75,
            "centrality": 0.65,
    ...
    def _generate_component_visualization(self, component_path: str) -> str:
        ...
        for conn in [c for c in self.connections if c["target"] == component_path]:
            mermaid.append(f"    {conn['source'].replace('/', '_')}[{conn['source']}] -->|{conn['type']}| COMP")
            
        # Add outgoing connections
        for conn in [c for c in self.connections if c["source"] == component_path]:
            mermaid.append(f"    COMP -->|{conn['type']}| {conn['target'].replace('/', '_')}[{conn['target']}]")
    ...
    def export_map(self, format: str = "json", path: Optional[Path] = None) -> Path:
        ...
        if format == "json":
            ...
            content = f"# EVA & GUARANI System Map\n\n" \
