...
class QuantumMycelium:
    ...
    def analyze_connections(self):
        ...
        self.disconnected_files = [
            f
            for f in self.files
            if len(self.file_references[f]) == 0 and len(self.reference_graph[f]) == 0
        ]
    ...
    def find_duplicates(self):
        ...
        files_to_check = [
            f
            for f in self.files
            if not self.file_metadata[f]["is_binary"]
            and self.file_metadata[f]["extension"] in IMPORTANT_EXTENSIONS
        ]

        # Limit check to avoid excessive processing
        if len(files_to_check) > 100:
            files_to_check = sorted(files_to_check, key=lambda f: self.file_metadata[f]["size"])[
                :100
            ]
        ...
        for file_path in self.files:
            ...
            for dup_group in self.duplicates:
                if file_path in dup_group:
                    ...
                    sorted_group = sorted(
                        dup_group, key=lambda f: self.file_metadata[f]["modified"]
                    )
    ...
    def recommend_actions(self):
        ...
        for dup_group in self.duplicates:
            if len(dup_group) > 1:
                ...
                sorted_group = sorted(
                    dup_group, key=lambda f: self.file_metadata[f]["modified"], reverse=True
                )
        ...
        try:
            ...
            if rec_type == "quarantine":
                ...
                extract_file = os.path.join(
                    STAGING_DIR, f"extract_{os.path.basename(recommendation['file'])}"
                )
                with open(extract_file, "w", encoding="utf-8") as f:
                    f.write(
                        f"""# Important content extracted from {recommendation['file']}
...
"""
    ...
    def generate_report(self, recommendations):
        ...
        with open(MYCELIUM_REPORT_FILE, "w", encoding="utf-8") as f:
            f.write(
                f"""# Mycelial Analysis Report - EVA & GUARANI
            ...
            json.dump(
                {
                    ...
                    "potentially_obsolete": [
                        (file, reasons) for file, reasons in self.potentially_obsolete
                    ],
