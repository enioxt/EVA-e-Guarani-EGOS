...
class SystemMaintenance:
    ...
    def _should_exclude(self, path: Path) -> bool:
        ...
        path_str = str(path)
        return any(
            pattern in path_str or (pattern.startswith("*") and path_str.endswith(pattern[1:]))
    ...
    def restore_backup(self, backup_name: str, password: Optional[str] = None) -> bool:
        ...
        try:
            ...
            temp_backup_name = (
                f"temp_before_restore_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
            temp_backup = self.create_backup(
                password="temp123", compress=True  # Use a temporary password
            )
    ...
    def cleanup_system(self) -> Dict[str, List[str]]:
        ...
        results = {"quarantined": [], "duplicates": [], "outdated": []}
    ...
    def _is_excluded(self, file_path: Path) -> bool:
        ...
        return (
            any(parent in excluded_dirs for parent in file_path.parents)
            or file_path.suffix in excluded_extensions
        )
    ...
    def get_system_stats(self) -> Dict:
        ...
        stats = {
            "total_files": 0,
            "total_size": 0,
            "recent_activity": {"modified_24h": 0, "modified_7d": 0, "modified_30d": 0},
            "file_types": {},
            "quarantine": {"total_files": 0, "total_size": 0},
            "backups": {"count": 0, "total_size": 0, "latest": None},
        }
    ...
    def reorganize_root_directory(self):
        ...
        try:
            ...
            quantum_files = {
                ...
                "QUANTUM_PROMPTS/ETHIK": ["prompt_encoder.py", "setup_session.txt"],
                "QUANTUM_PROMPTS/ATLAS": ["mycelium_monitor.js", "update_roadmap.js"],
                "QUANTUM_PROMPTS/CRONOS": [
                    "schedule_roadmap_update.js",
                    "start_monitor_simple.ps1",
                    "start_mycelium_monitor.ps1",
                ],
                "QUANTUM_PROMPTS/NEXUS": ["create_postman_collection.ps1"],
                "QUANTUM_PROMPTS/BIOS-Q": ["start_server_simple.ps1"],
            }
                    ...
                    if source_file.exists():
                        ...
                        try:
                            ...
                            results["moved_files"].append(
                                f"Moved to subsystem: {source_file} -> {target_file}"
                            )
                        ...
                        if source_file.is_file():
                            ...
                            try:
                                if target_file.exists():
                                    quarantine_path = (
                                        Path(self.root_dir) / "quarantine" / source_file.name
                                    )
                                    shutil.move(str(source_file), str(quarantine_path))
                                    results["moved_files"].append(
                                        f"Quarantined: {source_file} -> {quarantine_path}"
                                    )
                                else:
                                    shutil.move(str(source_file), str(target_file))
                                    results["moved_files"].append(
                                        f"Moved: {source_file} -> {target_file}"
                                    )
                            except Exception as e:
    ...
    def migrate_bios_q(self) -> Dict[str, Any]:
        ...
        """
        results = {"success": False, "files_moved": [], "errors": [], "preserved_files": []}
    ...
    def _update_bios_q_paths(self, bios_q_dir: Path):
        ...
        try:
            ...
            for py_file in bios_q_dir.glob("**/*.py"):
                ...
                # Atualizar caminhos
                content = content.replace(
                    'BIOS_Q_DIR = Path("BIOS-Q")', 'BIOS_Q_DIR = Path("QUANTUM_PROMPTS/BIOS-Q")'
                )
            ...
            for config_file in config_files:
                ...
                # Atualizar caminhos
                content = content.replace('path = "BIOS-Q"', 'path = "QUANTUM_PROMPTS/BIOS-Q"')
