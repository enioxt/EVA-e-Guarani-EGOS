...
class MemoryAnalyzer:
    ...
    def analyze_directory(self, directory: str) -> Dict[str, Any]:
        ...
        if not os.path.exists(directory):
            ...
            return {"exists": False, "files_count": 0, "total_size_bytes": 0, "files": []}

        dir_info = {"exists": True, "files_count": 0, "total_size_bytes": 0, "files": []}
                ...
                if "transactions" in data:
                    ...
                    for user_id, user_data in data.get("users", {}).items():
                        if "credits" in user_data:
                            credits_stats["total_special_calls"] += user_data["credits"].get(
                                "special_calls", 0
                            )
                            credits_stats["total_internet_calls"] += user_data["credits"].get(
                                "internet_calls", 0
                            )

                    if payments_info["users_count"] > 0:
                        credits_stats["avg_special_calls"] = (
                            credits_stats["total_special_calls"] / payments_info["users_count"]
                        )
                        credits_stats["avg_internet_calls"] = (
                            credits_stats["total_internet_calls"] / payments_info["users_count"]
                        )
    ...
    def analyze_conversations(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Conversation system analyzed: {conversations_info['total_conversations']} conversations"
        )
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "total_files": sum(
                info.get("files_count", 0) for info in self.results["memory_subsystems"].values()
            ),
    ...
    def _generate_text_report(self, filepath: str) -> None:
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"• Active subsystems: {summary.get('active_memory_subsystems', 0)} of {summary.get('total_memory_subsystems', 0)}\n"
            )
            ...
            f.write(
                f"• Average messages per conversation: {convs.get('avg_messages_per_conversation', 0):.2f}\n\n"
            )
