        ...
        if dest_path.exists():
            ...
            for item in source_path.iterdir():
                ...
                if item.is_dir():
                    if target.exists():
                        logging.warning(
                            f"Subdirectory {item.name} already exists in destination, skipping"
                        )
                    else:
...
def move_remaining_directories():
    ...
    logging.info(
        f"Processing of remaining directories completed: {moved_count} moved, {skipped_count} kept in root"
    )
...
def generate_report():
    ...
    backup_count = (
        len([d for d in backup_path.glob("**/*") if d.is_dir()]) if backup_path.exists() else 0
    )
    quarantine_count = (
        len([d for d in quarantine_path.glob("**/*") if d.is_dir()])
