...
class TelegramUIBuilder:
    ...
    def create_model_menu(
        ...
        for i in range(0, len(available_models), 2):
            ...
            line.append(
                InlineKeyboardButton(
                    f"{model['emoji']} {model['name']}", callback_data=f"model_{model['name']}"
                )
            )
    ...
    def create_confirmation(
        self, action: str, description: str, action_id: str
    ) -> Tuple[str, InlineKeyboardMarkup]:
    ...
    def create_gallery(
        self, items: List[Dict[str, Any]], index: int = 0
    ) -> Tuple[str, InlineKeyboardMarkup]:
        ...
        if index > 0:
            nav_buttons.append(
                InlineKeyboardButton("⬅️ Previous", callback_data=f"gallery_{index-1}")
            )
