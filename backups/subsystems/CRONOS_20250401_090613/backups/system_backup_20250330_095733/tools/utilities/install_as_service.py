...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/service_installer.log"), logging.StreamHandler()],
)
...
def install_nssm():
    ...
    try:
        ...
        nssm_path = os.path.join(
            os.environ.get("PROGRAMFILES", "C:\\Program Files"), "nssm", "nssm.exe"
        )
...
def install_service(service_name, display_name, description, script_path, nssm_path):
    ...
    try:
        ...
        subprocess.run(
            f'"{nssm_path}" set "{service_name}" DisplayName "{display_name}"',
            shell=True,
            check=True,
        )
        subprocess.run(
            f'"{nssm_path}" set "{service_name}" Description "{description}"',
            shell=True,
            check=True,
        )
        ...
        subprocess.run(
            f'"{nssm_path}" set "{service_name}" AppDirectory "{work_dir}"', shell=True, check=True
        )
        ...
        subprocess.run(
            f'"{nssm_path}" set "{service_name}" AppStdout "{stdout_log}"', shell=True, check=True
        )
        subprocess.run(
            f'"{nssm_path}" set "{service_name}" AppStderr "{stderr_log}"', shell=True, check=True
        )

        # Configure automatic restart
        subprocess.run(
            f'"{nssm_path}" set "{service_name}" AppRestartDelay 10000', shell=True, check=True
        )  # 10 seconds
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Install the EVA & GUARANI bot as a Windows service"
    )
    parser.add_argument("--remove", action="store_true", help="Remove the service")
    parser.add_argument(
        "--service-name", default="EVAGuaraniBot", help="Service name (default: EVAGuaraniBot)"
    )
    parser.add_argument(
        "--display-name", default="EVA & GUARANI Telegram Bot", help="Service display name"
    )
    parser.add_argument(
        "--script",
        default="monitor_bot.py",
        help="Script to be run as a service (default: monitor_bot.py)",
    )
