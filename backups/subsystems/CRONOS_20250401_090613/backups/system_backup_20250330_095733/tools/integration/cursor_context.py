...
try:
    ...
    logger.error("Failed to import PerplexityIntegration. Make sure the module is properly installed.")
...
class CursorContextEnricher:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.enabled:
            logger.warning("Cursor context enrichment not enabled. Missing API key or Perplexity integration.")
    ...
    async def enrich_context(self, 
        ...
        try:
            ...
            return {
                "status": "error",
                "error": f"Exception during context enrichment: {str(e)}"
            }
...
async def main():
    ...
    enrich_parser.add_argument("--name", "-n", help="Context name (defaults to 'perplexity_<timestamp>')")
    enrich_parser.add_argument("--persona", "-p", help="Persona to use (philosopher, scientist, etc.)")
