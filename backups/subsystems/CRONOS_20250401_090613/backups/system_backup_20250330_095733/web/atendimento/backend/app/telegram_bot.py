...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "chat":
        ...
        await query.edit_message_text(text="Descreva a imagem que você gostaria de criar:")
        return IMAGE_PROMPT

    elif query.data == "video":
        await query.edit_message_text(text="Descreva o vídeo que você gostaria de criar:")
        return VIDEO_PROMPT
        ...
        if not personas:
            await query.edit_message_text(text="Nenhuma persona disponível no momento.")
            return MAIN_MENU

        keyboard = []
        for persona_id, persona_name in personas:
            keyboard.append(
                [InlineKeyboardButton(persona_name, callback_data=f"select_persona_{persona_id}")]
            )
        ...
        await query.edit_message_text(
            text="Escolha uma persona para conversar:", reply_markup=reply_markup
        )
        ...
        await query.edit_message_text(
            text="O que você gostaria de fazer hoje?", reply_markup=reply_markup
        )
        return MAIN_MENU
...
async def chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Vamos conversar! O que você gostaria de discutir hoje?")
    ...
    try:
        ...
        if STABLE_DIFFUSION_AVAILABLE and stable_diffusion_api is not None:
            ...
            if success and image_data:
                ...
                await processing_msg.edit_text("✅ Imagem gerada e enviada com sucesso!")
        ...
        if (
            image_api_key
            and image_api_key != "sua_chave_api_aqui"
            and image_api_url
            and image_api_url != "https://api.exemplo.com/images"
        ):
...
async def handle_video_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if (
            video_api_key
            and video_api_key != "sua_chave_api_aqui"
            and video_api_url
            and video_api_url != "https://api.exemplo.com/videos"
        ):
...
async def persona_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    personas = ["Criativa", "Científica", "Filosófica", "Estratégica", "Educacional", "Artística"]

    keyboard = []
    for i in range(0, len(personas), 2):
        row = []
        row.append(
            InlineKeyboardButton(personas[i], callback_data=f"select_persona_{personas[i].lower()}")
        )
        if i + 1 < len(personas):
            row.append(
                InlineKeyboardButton(
                    personas[i + 1], callback_data=f"select_persona_{personas[i+1].lower()}"
                )
            )
        keyboard.append(row)
    ...
    await update.message.reply_text(
        "Escolha uma persona para conversar:", reply_markup=reply_markup
    )
    return SELECTING_PERSONA
    ...
    try:
        ...
        if result.get("status") == "success":
            ...
            if len(response_text) > 4000:
                ...
                await update.message.reply_text(
                    response_text, parse_mode=ParseMode.MARKDOWN, disable_web_page_preview=True
                )
        else:
            # Handle error
...
async def main():
    ...
    if not TELEGRAM_TOKEN:
        logger.error(
            "Token do Telegram não encontrado. Configure a variável TELEGRAM_TOKEN no arquivo .env"
        )
        return
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            SELECTING_PERSONA: [CallbackQueryHandler(button_handler)],
        },
