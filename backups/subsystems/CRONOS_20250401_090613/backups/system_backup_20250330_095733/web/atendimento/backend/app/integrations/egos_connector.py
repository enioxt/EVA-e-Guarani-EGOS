...
try:
    ...
    logger.warning(
        "Módulo ATLAS não encontrado. Funcionalidades de cartografia sistêmica serão limitadas."
    )
...
try:
    ...
    logger.warning(
        "Módulo NEXUS não encontrado. Funcionalidades de análise modular serão limitadas."
    )
...
try:
    ...
    logger.warning(
        "Módulo ETHIK não encontrado. Funcionalidades de avaliação ética serão limitadas."
    )
...
try:
    ...
    logger.warning(
        "Módulo CRONOS não encontrado. Funcionalidades de preservação evolutiva serão limitadas."
    )
...
class EGOSConnector:
    ...
    def __init__(self, data_dir: Optional[str] = None):
        ...
        logger.info(
            f"EGOSConnector inicializado. Subsistemas disponíveis: {self.available_systems}"
        )
    ...
    def evaluate_message_ethics(self, message: str) -> Dict[str, Any]:
        ...
        if not self.available_systems["ethik"]:
            ...
            return {"aligned": True, "score": 0.75, "concerns": [], "evaluation_method": "fallback"}
    ...
    def analyze_user_preference(
        self, user_id: int, chat_history: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            user_data = {"id": user_id, "history": chat_history}
    ...
    def save_conversation_state(
        ...
        if not self.available_systems["cronos"]:
            ...
            try:
                ...
                return {"success": False, "error": str(e), "method": "fallback"}
        ...
        try:
            ...
            try:
                ...
                return {"success": False, "error": str(e2), "method": "error_fallback"}
    ...
    def list_available_personas(self) -> List[Dict[str, Any]]:
        ...
        telegram_personas_dir = (
            Path(__file__).resolve().parents[2] / "data" / "telegram_bot" / "personas"
        )
            ...
            if full_path.exists():
                ...
                if full_path.suffix.lower() == ".json":
                    try:
                        ...
                        logger.error(
                            f"Erro ao carregar persona {persona_id} de {full_path}: {str(e)}"
                        )
            ...
            if full_path.exists():
                ...
                # Para arquivos Markdown
                    ...
                    try:
                            ...
                            for line in lines:
                                if "**User:**" in line:
                                    if example_user and example_assistant:
                                        examples.append(
                                            {"user": example_user, "assistant": example_assistant}
                                        )
                                        example_user = ""
                                        example_assistant = ""
                        ...
                        logger.error(
                            f"Erro ao converter markdown para persona {persona_id} de {full_path}: {str(e)}"
                        )
