...
class MyceliumNetwork:
    ...
    def load_from_quantum_mycelium(self):
        ...
        print(
            f"✅ Created network with {self.graph.number_of_nodes()} nodes and {self.graph.number_of_edges()} edges"
        )
    ...
    def analyze_network(self):
        ...
        try:
            ...
            try:
                ...
                print(
                    "⚠️ Community detection requires either NetworkX community algorithms or python-louvain package."
                )
                print(
                    "⚠️ Community detection will be skipped. Visualization will use file types for coloring instead."
                )
    ...
    def visualize_network(self, max_nodes=150):
        ...
        if self.graph.number_of_nodes() > max_nodes:
            print(
                f"⚠️ Network is large ({self.graph.number_of_nodes()} nodes). Creating simplified view."
            )
        ...
        if self.communities:
            ...
            node_colors = [
                ext_colors.get(os.path.splitext(node)[1], "black") for node in vis_graph.nodes()
            ]

        # Set node size based on centrality or file size
        if hasattr(self, "centrality") and self.centrality.get("betweenness"):
            node_sizes = [
                5000 * self.centrality["betweenness"].get(node, 0.1) + 50
    ...
    def _export_network_data(self):
        ...
        network_data = {
            ...
            "metadata": {
                "timestamp": (
                    str(Path(NETWORK_FILE).stat().st_mtime)
    ...
    def generate_report(self):
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            f.write(
                """# Mycelium Network Analysis Report
...
"""
            ...
            if weak_components:
                ...
                f.write(
                    f"- **Largest Component Size**: {len(largest_component)} files ({len(largest_component)/self.graph.number_of_nodes()*100:.1f}% of total)\n\n"
                )
            else:
            ...
            if hasattr(self, "centrality") and self.centrality.get("betweenness"):
                top_central = sorted(
                    self.centrality["betweenness"].items(), key=lambda x: x[1], reverse=True
                )[:10]
            ...
            if self.recommendations:
                for i, rec in enumerate(self.recommendations):
                    ...
                    if rec["type"] == "structural_bridge":
                        ...
                        f.write(
                            f"**Files**: {', '.join(rec['files'][:3])}"
                            + (f" and {len(rec['files'])-3} more" if len(rec["files"]) > 3 else "")
                            + "\n\n"
                        )
                        f.write(
                            "This group of files forms an isolated component that isn't well connected to the rest "
                        ...
                        f.write(
                            "This file has an unusually high number of outgoing dependencies, which may indicate "
            ...
            f.write("\n## Visualization\n\n")
            f.write(
                f"A network visualization has been saved to: `{os.path.relpath(NETWORK_IMAGE, PROJECT_ROOT)}`\n\n"
            )

            # Add JSON data reference
            f.write(
                f"Interactive visualization data available at: `{os.path.relpath(NETWORK_FILE, PROJECT_ROOT)}`\n\n"
            )
