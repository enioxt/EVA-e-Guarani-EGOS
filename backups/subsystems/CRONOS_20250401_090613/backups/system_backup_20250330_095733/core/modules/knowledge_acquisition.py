...
DEFAULT_CONFIG = {
    "repositories": {
        "arxiv": {"enabled": True, "max_results": 100, "query_limit_per_day": 1000},
        "plos": {"enabled": True, "max_results": 100, "api_key": ""},
        "pubmed": {"enabled": True, "max_results": 100, "api_key": ""},
        "scielo": {"enabled": True, "max_results": 100},
        "doaj": {"enabled": True, "max_results": 100},
        "gutenberg": {"enabled": True, "max_results": 50},
    },
    "perplexity": {
        "enabled": False,
        "api_key": "",
    ...
    for connection in new_connections:
        ...
        for existing in mapping["connections"]:
            if (
                ...
                and existing["target"] == connection["target"]
            ):
...
def calculate_domain_similarity(domain1, domain2):
    ...
    domain1_connections = [
        c for c in mapping["connections"] if c["source"] == domain1 or c["target"] == domain1
    ]
    domain2_connections = [
        c for c in mapping["connections"] if c["source"] == domain2 or c["target"] == domain2
    ]
    ...
    if common_domains:
        ...
        for common in common_domains:
            strength1 = next(
                c["strength"]
                for c in domain1_connections
                if (c["source"] == common or c["target"] == common)
            )
            strength2 = next(
                c["strength"]
                for c in domain2_connections
                if (c["source"] == common or c["target"] == common)
            )
...
def search_pubmed(query, max_results=50, api_key=None):
    ...
    try:
        # Set up parameters
        params = {"term": query, "retmax": max_results, "retmode": "json"}
        ...
        fetch_params = {"db": "pubmed", "id": id_param, "retmode": "xml"}
    ...
    try:
        ...
        data = {
            "model": (
                ...
                if mode == "thorough"
                else "llama-3-sonar-large-online"
            ),
            "messages": [
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(sources)) as executor:
        ...
        for source in sources:
            if source == "arxiv" and config["repositories"]["arxiv"]["enabled"]:
                ...
                search_tasks[executor.submit(search_arxiv, query, max_arxiv, arxiv_categories)] = (
                    "arxiv"
                )
                ...
                search_tasks[executor.submit(search_open_access, query, "scielo", max_scielo)] = (
                    "scielo"
                )
    ...
    if "perplexity" in sources and config["perplexity"]["enabled"]:
        try:
            perplexity_result = perplexity_api_search(
                query, mode=config["perplexity"]["research_mode"]
            )
...
def save_knowledge_source(source_data, content=None, file_path=None):
    ...
    save_source_metadata(
        source_id,
        {
            ...
            "acquisition_date": source_data.get(
                "acquisition_date", datetime.datetime.now().isoformat()
            ),
        },
    )
