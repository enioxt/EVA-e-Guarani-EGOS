...
class Translator:
    ...
    def scan_directory(self, input_dir: str, target_lang: str = "pt") -> ScanResult:
        ...
        with self.console.status(
            "[bold green]Scanning for Portuguese files...[/bold green]"
        ) as status:
        ...
        if scan_result.files:
            self.console.print(
                f"[green]Found {len(scan_result.files)} Portuguese files to translate[/green]"
            )
        else:
    ...
    def translate(
        ...
        self.console.print(
            f"[bold]Translating {len(scan_result.files)} files to:[/bold] {output_dir}"
        )
...
def main():
    ...
    parser.add_argument(
        "--translate", metavar="DIR", help="Translate Portuguese files in directory"
    )
    ...
    parser.add_argument(
        "--workers", type=int, default=2, help="Number of worker threads (default: 2)"
    )
    parser.add_argument(
        "--cache-dir", metavar="DIR", help="Cache directory (default: data/cache/translations)"
    )
    ...
    if args.translate:
        ...
        if scan_result.files:
            translator.translate(
                scan_result, output_dir=output_dir, engine_name=args.engine, workers=args.workers
            )
        return
