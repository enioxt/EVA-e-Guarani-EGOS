...
class QuantumRoadmapManager:
    ...
    def scan_documentation_files(self):
        ...
        logging.info(f"{Fore.BLUE}Scanning documentation files for cross-references...{Style.RESET_ALL}")
        ...
        logging.info(f"{Fore.GREEN}Documentation analysis complete. Found {len(self.doc_references)} files with references.{Style.RESET_ALL}")
    ...
    def compare_with_previous_state(self):
        ...
        for doc in current_docs & previous_docs:
            ...
            if current_modified != previous_modified:
                current_refs = set(current["documentation_files"][doc].get("documentation_refs", []))
                previous_refs = set(previous["documentation_files"][doc].get("documentation_refs", []))
        ...
        try:
            ...
            with open(roadmap_path, 'w') as f:
                ...
                for doc_file, refs in self.doc_references.items():
                    doc_refs = ", ".join([os.path.basename(r) for r in refs.get("documentation_refs", [])])
                    impl_refs = ", ".join([os.path.basename(r) for r in refs.get("implementation_refs", [])])
...
def main():
    ...
    logging.info(f"{Fore.CYAN}To access the state preservation system, use: python -m BIOS-Q.bios_q.cronos.state_preservation{Style.RESET_ALL}")
