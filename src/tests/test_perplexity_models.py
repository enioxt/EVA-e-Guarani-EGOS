...
def api_key() -> str:
    ...
    key = get_api_key()
    if not key:
        pytest.skip("API key not found. Set PERPLEXITY_API_KEY in the environment or in the configuration file.")
    return key
...
def test_model_availability(model: str, api_key: str, client: Any) -> None:
    ...
    try:
        ...
        assert "Brazil" in content or "Brasília" in content, f"Response from model {model} does not seem to answer the question about the capital of Brazil"
...
def save_test_result(
    ...
    result_file = report_dir / f"perplexity_test_{model}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
